...
{
    return heightMapParams.x*texture(image47, (uv + TIME*heightMapParams.zw)*heightMapParams.y).x;
}

float bumpmap(vec2 uv)
{
    float b1 = bumpMapParams1.x*(1.0 - texture(image47, (uv + smoothTimeC*bumpMapParams1.zw)*bumpMapParams1.y).x);
    float b2 = bumpMapParams2.x*(1.0-texture(image47, (uv + smoothTimeC*bumpMapParams2.zw)*bumpMapParams2.x).x);
    return b1+b2;
}
...
float distfunc(vec3 pos)
{
    ...
    vec3 reppos = (mod(p2 + vec3(smoothTimeC*0.01 + sin(pos.z*0.5),0.0,0.0),columnsrep)-0.5*columnsrep);
...
vec4 renderPassA() {
    ...
    vec3 cameraOrigin = vec3(0.0, 0.0, TIME*-0.1);
    vec3 cameraTarget = cameraOrigin + vec3(0.0, 0.0, 1.0);;
    
	vec2 screenPos = (fragCoord.xy/RENDERSIZE.xy)*2.0-1.0;
    ...
    float m = smoothstep(-0.15,0.2, planesDistance - abs(uv.y) + h*heightInfluence.y + sin(smoothTime)*0.05);
...
vec4 renderPassB() {
    ...
    vec4 tex = vec4(0.0);
    vec2 dist = sampleDist/RENDERSIZE.xy;
    
    for(int x = -2; x <= 2; x++)
    {
    	for(int y = -2; y <= 2; y++)
        {
			tex += texture(BuffA, uv + vec2(x,y)*dist);
        }
    }
...
vec4 renderPassC() {
    ...
    for(int x = -2; x <= 2; x++)
    {
    	for(int y = -2; y <= 2; y++)
        {
			tex += texture(BuffB, uv + vec2(x,y)*dist);
        }
    }
...
vec4 renderPassD() {
    ...
    for(int x = -2; x <= 2; x++)
    {
    	for(int y = -2; y <= 2; y++)
        {
			tex += texture(BuffC, uv + vec2(x,y)*dist);
        }
    }
...
vec4 renderMainImage() {
    ...
    vec4 tex = texture(BuffA, uv);
    vec4 texBlurred = texture(BuffD, uv);
