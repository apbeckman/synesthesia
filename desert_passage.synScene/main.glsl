...
float cellTile(in vec3 p){
    ...
    d.xy *= Rock_Morph;
    d.xy += .7*sin(syn_BassTime*.2)*Morph_Time;
...
float map(vec3 p){
  ...
  //  float tun = 1.5 - length(p.xy*vec2(1, .4));
    float tun = Tunnel_Width - length(p.xy*vec2(1, .4));
...
float trig3(in vec3 p){
    p = cos(p*2. + (cos(p.yzx) + 1.)*1.57);// + TIME*1.
...
vec4 renderMainImage() {
	...
	// Camera Setup.
	vec3 ro = vec3(0, 0, syn_Time*.25*8.); // Camera position, doubling as the ray origin.
	vec3 lookAt = ro + vec3(LookXY*Cam_Control, .5);  // "Look At" position.
    ...
    if (t < FAR){
        ...
        sn = doBumpMap(image45, sp*tSize, sn, bf/(1. + t/FAR));
        ...
        col = smoothstep(-.5, 1., tex3D(image45, sp*tSize, sn)*2.)*(col + vec3(.225, .21375, .19125));
