...
function update(dt) {
  ...
  var warp_speed = inputs.Warp_Gain;
  var plane_speed = inputs.Plane_Gain;
  ...
  try {
    ...
    warpTimeVar.updateTime(0.5, inputs.Warp_Time > 0.5 ?
    (inputs.syn_Level*2.0 + inputs.syn_Presence + inputs.syn_Hits*1.0 + inputs.syn_BassHits*4.0)*warp_speed : 0, dt);

    planeTimeVar.updateTime(1.0, (inputs.syn_BassLevel*1.0 + inputs.syn_BassHits*2.0)*plane_speed, dt);

    uniforms.fly_time = flyTimeVar.time;
    uniforms.warp_time = warpTimeVar.time;
    uniforms.plane_time = planeTimeVar.time;
