...
vec3 blackbodyPalette(float t){
    ...
    // t = tLow + (tHigh - tLow)*t;
    t *= Light_Brightness*4000.; // Temperature range. Hardcoded from 0K to 4000K, in this case.

    t += t*Reactive_Temp*syn_BassHits*.5;
...
float raymarch(vec3 ro, vec3 rd) {
    ...
    int numSteps = int(128*pow(Ray_Steps,30));
...
float softShadow(vec3 ro, vec3 rd, float start, float end, float k){
    ...
    float shade = 1.0;
    const int maxIterationsShad = 32; // 24 or 32 would be better.
...
vec4 renderMainImage() {
    ...
    rd.xy *= rot(syn_Time*0.25*Rotation);
    rd.xz *= rot(syn_Time*0.125*Rotation);
    ...
    vec3 ro = vec3(0.0, 0.0, fly_time);
    ro.xy += CamPosition;
    ro.xy += vec2(-1*Circle_Tunnel,1*Circle_Tunnel);
    //vec3 ro = vec3(0.5 + TIME*0.7, 0.0, TIME*0.7); // Another lattice traversal cliche.

    // Light position. Rotated a bit, then placed a little above the viewing position.
    vec3 lp = vec3(0.0, 0.125, -0.125);
    lp.xy *= rot(TIME*0.5);
    lp.xz *= rot(TIME*0.25);
    lp += ro + vec3(0.0, 1.0, syn_Level*.5);
    ...
    if (dist < FAR){
	    ...
	    vec3 objCol = tex3D( image3, sp, sn );
        ...
        float refShade = getGrey(tex3D( image7, sp/4. + TIME/64., sf ));
