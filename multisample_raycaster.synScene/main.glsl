...
float h(vec2 p){
    ...
    vec3 tx = texture(image46, p/RENDERSIZE.xy*4.).xyz;  tx *= tx;///iChannelResolution[0].xy
    // Greyscale height. Using "tx.x" would work, too.
	float f = dot(tx, vec3(.299, .587, .114));
    float f2 = f;
    
    //f = sin(f*6.2831 + TIME)*.5 + .5;
...
vec4 renderPassA() {
    ...
    vec2 uv0 = (fragCoord - RENDERSIZE.xy*.5)/RENDERSIZE.y;
    

    // Initializing the seed value. It needs to be different every frame.
    seed = uv0 + vec2(fract(TIME/113.671)*.123, fract(TIME/57.913)*.14527);
    
    // Ray origin.
    vec3 ro = vec3(-s.x/2., 3.5, TIME*.25); 
    ...
    mCam *= rot(vec3(0, 0, cos(TIME/8.*.25)/4.)); // Camera yaw.
    mCam *= rot(vec3(-sin(TIME/4.*.25)/8., 0, 0)); // Camera roll.
    mCam *= rot(vec3(0, .5, 0)); // Y axis tilt, or pitch.
    
/*
    // Artistic black movie strips. :)
    if(abs(uv0.y)>.35) { 
        ivec2 q = ivec2(u);
        c = vec4(0, 0, 0, 1); 
    	if(q.y == 0 && q.x<3){
...
*/
    ...
    for(int j = min(0, int(FRAMECOUNT)); j<sampNum; j++){
        ...
        for(int i = min(0, int(FRAMECOUNT)); i<3; i++){
            ...
            if(t<FAR - 1e-6){
                ...
                vec3 tx = texture(image3, rot2(3.14159/3.*0.)*tuv/3.).xyz; tx *= tx;
                ...
                //if(hash21(id +.1)<.25 && strip<0.){
                    ...
                    float blink = smoothstep(.2, .3, sin(6.2831*hash21(tip + .09) + TIME/4.)*.5 + .5);
                ...
                float rChance = step(0., rough - hash21(uv + vec2(i*277, j*113) + fract(TIME*.97 + .137)));
                rd = (mix(ref, rrd, rChance));
                // Other variations. Not physically correct, but they have their purposes.
                //float rChance = rough*hash21(uv + vec2(i*277, j*113) + fract(TIME*.97 + .137));
                ...
                // If the scene hasn't been hit, add a touch of atmospheric haze, then QaZ
                acc = vec3(.4, .6, 1)*.035; 
                break;
            }
    ...
    mat3x4 oldCam = mat3x4(texelFetch(BuffA, ivec2(0, 0), 0),
                           texelFetch(BuffA, ivec2(1, 0), 0),
                           texelFetch(BuffA, ivec2(2, 0), 0));
    // World space point.
    vec4 wpos = vec4(pos, 1.);
    // Convert to camera space (note inverse multiply).
    vec3 cpos = wpos*oldCam;
    // Convert to NDC space (project).
    vec2 npos = (kFocLen*2.)*cpos.xy/cpos.z;//*iRes/RENDERSIZE.y;
    // Convert to screen space.
    vec2 spos = .5 + .5*npos*vec2(RENDERSIZE.y/RENDERSIZE.x, 1);
	// Convert to raster space.
    vec2 rpos = spos*RENDERSIZE.xy;

    // Read color+depth from this point's previous screen location.
    vec4 ocolt = textureLod( BuffA, spos, 0.);
    // If we consider the data contains the history for this point.
    if(FRAMECOUNT>0 && resT<FAR && (rpos.y>1.5 ||rpos.x>3.5)){
    ...
    float blend = (FRAMECOUNT < 2) ? 1. : 1./4.; 
...
vec4 renderMainImage() {
    ...
    //
    float maxRes = RENDERSIZE.y;//540.;
    vec2 uv = fragCoord/RENDERSIZE.xy;
    // If the resolution exceeds the maximum, upscale.
    if(RENDERSIZE.y>maxRes) uv = (fragCoord/RENDERSIZE.xy - .5)*maxRes/RENDERSIZE.y + .5;
