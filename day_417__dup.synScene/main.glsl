...
vec4 renderPassA() {
    ...
    float n = cyclicNoise(vec3(uv*5.,smoothTime*0.5));
    
    n = cyclicNoise(vec3(uv*10. + n*1.15,smoothTime));
        
        
    float nb = cyclicNoise(vec3(uv*5.,smoothTime));
        
        
    
    
    float vn = valueNoise(smoothTimeB/2.,5.);
    float vnb = valueNoiseStepped(smoothTime/2. + n*0.4*pow(vn,4.) + 5.,2.,4.);
    float vnc = valueNoiseStepped(smoothTime/8.,4.,4.);
    ...
    col += smoothstep(dfuv*(1. + sin(smoothTime + uv.x)*0.4 + 0.4),0.,d - lineW);
    
    
    if(floor(mod(smoothTime/2.,2.)) == 0. ){
        col = 1. - col;
    }
    ...
    col = mix(col, 1. - col, 
        step(0.6,valueNoiseStepped(TIME*25.,2.,4.)/20.) *
        step(0.2,valueNoiseStepped(TIME*2.5,2.,2.)/20.)
