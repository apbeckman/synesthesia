...
#define FAR pow(Fog,3)

// 2x2 matrix rotation. Note the absence of "cos." It's there, but in disguise, and comes courtesy
// of Fabrice Neyret's "ouside the box" thinking. :)
// MS: Controls the overall scene, not really fun to mess with..
mat2 rot2( float a ){ vec2 v = sin(vec2(1.570796, 0) - a);
...
float drawObject(in vec3 p){
    ...
    p = abs(fract(p)-.5 + .075*Warp_Geometry*sin(syn_BassTime));
    return dot(p, vec3(Smoother));
...
float cellTile(in vec3 p){
    ...
    vec4 v, d;
    float pMorph = .7071 + Plane_Morph;

    d.x = drawObject(p - vec3(.81, .62, .53));
    p.xy = vec2(p.y-p.x, p.y + p.x)*pMorph;
    d.y = drawObject(p - vec3(.39, .2, .11));
    p.yz = vec2(p.z-p.y, p.z + p.y)*pMorph;
    d.z = drawObject(p - vec3(.62, .24, .06));
    p.xz = vec2(p.z-p.x, p.z + p.x)*pMorph;
...
vec3 cellTileColor(in vec3 p){
    ...
    vec3 col = hueShift(vec3(1, .05, .15),Second_Tile+3.14);//vec3(.7, .8, 1);

    if (Color_Secondary > 0.5) col = vec3(.25);


    if (cellID == 3) col = hueShift(vec3(1, .05, .15),Tile_Color);
    ...
    if (Hue_Rotate > 0.5) col = hueShift(col, sin(syn_Time*.1));
...
float map(vec3 p){
    ...
    p.y += sin(p.z/Wave_Length + syn_Time)*Wave_Height; // MS: Audioreactive sinusoidal wave function
    return p.y + dot(sin(p/2. + cos(p.yzx/2. + 3.14159/2.)), vec3(.5)) + n - Height_Bounce*syn_Level*cos(syn_Time) + Cam_Height; // MS: Makes it bounce a little bit
...
vec3 getNormal(in vec3 p) {
	vec2 e = vec2(0.005+syn_BassLevel*Shiny, 0);
...
vec3 texBump( sampler2D tx, in vec3 p, in vec3 n, float bf){
    ...
    return normalize( Tex_Bump + n + g*bf ); // Bumped normal. "bf" - bump factor.
...
// MS: To be clear, this is the camera path
...
vec4 renderMainImage() {
	...
	// Camera Setup.
	vec3 lk = vec3(0, 3.5, syn_Time*Fly_Speed*6.);  // "Look At" position.
	vec3 ro = lk + vec3(LookXY.x*-1*Cam_Control, LookXY.y*-.1*Cam_Control, -.25); // Camera position, doubling as the ray origin.
    ...
    float FOV = 1.57; // FOV - Field of view. MS: Def map this to slider, high FOV looks coooooool
	...
	*/
	...
	if(t < FAR){
	    ...
	    sn = texBump(image46, sp*tSize0, sn, 0.01);
