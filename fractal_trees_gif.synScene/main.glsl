...
{
	const vec3 eps = vec3(0.0025, 0.0, 0.0);
...
vec4 renderMainImage() {
    ...
    uv.y -= uv.x*uv.x*0.15;
    vec3 camPos = vec3(3., -1.5, smoothTimeC*2.5);
    vec3 camDir = camPos+vec3(-1.25,0.1, 1.);
    mat3 cam = camera(camPos, camDir, 0.);
    vec3 rayDir = cam * normalize( vec3(uv, .8));
        rayDir.yz = _rotate(rayDir.yz, lookXY.y*PI);
    rayDir.xy = _rotate(rayDir.xy, -1.0*lookXY.x*PI);
	...
	fragColor = vec4(1.-col.xyz,clamp(1.5 -col.w/MAXDIST, 0., 1.));
	return fragColor; 
 } 
