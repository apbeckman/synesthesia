...
vec4 valueNoise(float i, float tol){
    vec4 a = texture(image30,vec2(floor(i)*0.001));
    vec4 b = texture(image30,vec2(floor((i+1.))*0.001));
...
vec2 sUv = vec2(0);
#define mx (smoothTime*1. + 10.*iMouse.x/RENDERSIZE.x)
vec3 sumonTheDemon( vec2 p, float id )
{
    ...
    float rA = texture(image30, vec2(sin(id*0.01)*100.,id )).x;
    float rB = texture(image30, vec2(sin(id*0.03 + 0.2)*100.,id*1.4 )).x;
    ...
    for(int i = 0; i < 7; i++){
        ...
        p -= 0.05 + sin(smoothTime*0.1 + id*0.1)*0.0;
    //    p.x -= 0.008;
    }
    #endif
    ...
    p.x += valueNoise(smoothTimeC*1.5 + idy*20.5 + id, 0.1).x*0.04;
    p = abs(p);
    p *= rot(0.25*pi);
    if(rA < 0.5){
    	p = abs(p);
...
vec4 renderMainImage() {
    ...
    #define chromaticAbberationAmt (0.00 + sin(TIME)*0.005)
