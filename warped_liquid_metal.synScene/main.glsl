...
float onoise(vec2 x) {
  ...
  float a = sin(x.x);
  float b = sin(x.y);
  float c = mix(a+cos(smoothTimeB*0.01), b+sin(smoothTimeB*0.01), psin(TAU*tanh(a*b+a+b)));
...
float fbm2(vec2 p) {
  ...
  float twitch = (1.0+basshits*Twitch);
  return mix((h/d), -0.5*(h/d), pow(vnoise(0.9*op+smoothTime*0.01), 0.25*twitch));
}
...
float warp(vec2 p) {
  ...
  rot(v, 1.0+((smoothTimeC)*0.125));
  
  vec2 vv = vec2(fbm2(p + 3.7*v), fbm2(p + -2.7*v.yx+0.7*vec2(1.0, 1.0)));

  rot(vv, -1.0+smoothTimeC*0.21315);
    
  return fbm3(p + 1.4*vv);
}

float height(vec2 p) {
  float a = 0.0025*smoothTime;
