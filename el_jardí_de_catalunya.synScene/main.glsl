vec4 iMouse = vec4(MouseXY*RENDERSIZE, MouseClick, MouseClick); 
 ...
 vec2 raygl_sdf_map_1(in vec3 p) {
   ...
   for(int i = 0; i < 4; ++i) {
     ...
     if(r > 2.0) continue;
     theta = atan(z.y / z.x) ;
     phi = asin(z.z / r)  + smooth_hightime * 0.4  + 0.9 * mix(raygl_BMOD_1.x,1. - raygl_BMOD_1.y, 0.66 + r4.x * 0.33);
 ...
 float raygl_shape_1(in vec2 fragCoord, out vec3 pt, out vec3 nor) {
   ...
   vec3 ro = vec3(4. + TIME * 0.25, 1.7 , 2.9);
   mat3 cam = raygl_lookAt_2(ro, ro + vec3(4.,-6. ,0.), 0.);
   vec2 p = (-RENDERSIZE.xy + 2.0*fragCoord)/RENDERSIZE.y;
 ...
 vec4 renderMainImage() {
   ...
   z =  raygl_fbm_(pt.xz * 0.26 + vec2(0.01, 0.02) * smooth_basstime + vec2(0.2 * bms.y,0.2 * bms.z)  + q);
