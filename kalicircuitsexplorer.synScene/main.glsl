...
//
// based on :
// shadertoy.com\/view\/XlX3Rj  by Kali
//
...
float S = (101.0 + glow) * (intensity/2.);
vec3 color = vec3(0.0);
...
void formula(vec2 z, float t) 
{
	...
	float K = floor(loops/4.0)+floor(6.0 * zoom);
	for (int i=0; i<11; i++) {
	...
	color += col * (0.4 + mod(M / 9.0 - t * (pulse) + (ot2- (smooth_basstime*0.005)) * 2.0, 1.0));
...
vec4 renderMain() { 
	...
	float R = 0.0;
	float N = ((script_time*0.95) + (rate*0.75)) * 0.01;
	float T = 1.5 * rate;
//	T += (script_bass_time*0.0006);
