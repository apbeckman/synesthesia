...
#define THRUST 0.04*0.016667
#define ROLL 1.0*0.016667
#define ROTATE 4.0*0.016667
...
vec4 load(in int re) {
    return texture(BuffA, (0.5+vec2(re,0.0)) / RENDERSIZE.xy, -100.0 );
}
...
vec4 renderPassA() {
	...
	if(FRAMECOUNT<2){
		...
		vec2 mous=vec2(0.0);
		if(iMouse.z>0.0){
			mous.xy=(iMouse.xy-iMouse.zw)/RENDERSIZE.xy;
		}
...
float rnd;
void randomize(in vec2 p){rnd=fract(float(FRAMECOUNT)+sin(dot(p,vec2(13.3145,17.7391)))*317.7654321);}
...
vec3 scene(in vec3 ro, in vec3 rd){
	float d=DE(ro)*rnd*0.5,t=d,od=1.0,pxl=2.0/RENDERSIZE.y;
...
*/
vec4 load(in int re) {
    return texture(BuffA, (0.5+vec2(re,0.0)) / iChannelResolution[0].xy, -100.0 );
}
...
vec4 renderMainImage() {
	...
	randomize(fragCoord);
	vec3 rd=normalize(vec3((2.0*fragCoord-RENDERSIZE.xy)/RENDERSIZE.y,1.0));
	vec3 ro=load(0).xyz;
	quat fw=load(1);
	rd=qmulv(fw,rd);
