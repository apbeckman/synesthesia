...
vec4 renderMain(){
	...
	if(PASSINDEX == 2){
	    ...
	    vec4 f = FRAMECOUNT < 3 ? texture(colornoise, _uv) :  abs(edgeDetectSobel(rVertMirrPass))*0.95;//
        // f.a *= 0.05;
        return f;
	}
        ...
        float offset = 10.0/RENDERSIZE.x;
        ...
        fluidFB /= mix(12.0, 2.0, mix(flow, syn_Presence, auto_flow));
    ...
    if(PASSINDEX == 9.0) {
        ...
        spec=pow(spec,mix(64.0, 2.0, 0.0))*mix(2.5, 1.0, 0.0);
