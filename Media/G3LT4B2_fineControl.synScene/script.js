...
function update(dt) {
  ...
  try {
    ...
    bassTimevar.updateTime(0.2*inputs.syn_BassPresence, (inputs.syn_BassLevel*2.0+inputs.syn_BassPresence+inputs.syn_BassHits*2.0)*3.0, dt);
    ...
    uniforms.show_media = inputs.beat_flash > 0.5 ? Math.floor(inputs.syn_OnBeat*10.5)*0.1 : inputs.level_media > 0.5 ? inputs.syn_MidLevel*inputs.syn_Presence*0.25 : inputs._show_media > 0.5 ? 0.2   : inputs.hint_media > 0.5 ? inputs.syn_Presence*inputs.syn_Intensity*0.125 : 0.0;//+(inputs.syn_BassLevel + inputs.syn_BassPresence)/2.0*0.0625 + inputs.syn_MidLevel*0.03 : 0.0; 
