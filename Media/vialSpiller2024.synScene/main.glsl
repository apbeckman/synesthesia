...
vec4 eds(sampler2D tex){
    float stepSize = mix(0.2, 1.75, mix(0.0, 0.75+0.25*syn_Level, mix(alt_fb, alt_av, alt_auto)));
    vec2 uv = _uv;
...
vec4 renderMain() {
    ...
    if(PASSINDEX == 1){
        vec2 uv = _uv;
        uv = mc(uv);
        uv = mix(uv, mix(uv, fract(uv*(1.0+floor(syn_RandomOnBeat*3.5))), floor(syn_ToggleOnBeat*1.5)), tile_media);
        ...
        uv = mix(uv, puv, mix(pixelate, syn_BassLevel*syn_BassPresence, bass_pix));
	...
	if(PASSINDEX == 2){
        ...
        float squareBlast = _sqPulse(abs(c_uv.x), 1.0-syn_MidLevel, s*mix(0.2, 1.0, _size)+s*syn_MidLevel);
        ...
        vec2 uwu = ( ( _uv - 0.5 ) / ( 1.0 - mix(0.00, 0.000625, syn_BassPresence)*(1.0 - show_media)*syn_Presence) + 0.5 + viewPort);
