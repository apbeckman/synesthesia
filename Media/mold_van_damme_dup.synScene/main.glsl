...
float sangl = 0.2*(1.0+sin(smoothTime*0.1)*0.9); //radians
...
vec4 renderPassA() {
   ...
   // CheckRadius(U, pos, 5.);
    ...
    //vec2 pvel = pspeed*vec2(cos(U.z), sin(U.z))*(0.8+basshits/5.) + 0.1*(hash22(U.xy+smoothTime*0.3)-0.5);;
...
vec4 renderPassB() {
    ...
    Q += dt*(distr+2. * video * 1.0+(highhits*.25*flash)*syn_Intensity)*1.1;
    //Q += dt*distr/(1.);
    //Q+= basshits/1.;
    //pheromone decay
    Q += -dt*decay*Q;
