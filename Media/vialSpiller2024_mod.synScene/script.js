...
function update(dt) {
  try {
    ...
    bassTimevar.updateTime(
      0.1 + 0.1 * _size + 0.025 * inputs.syn_BassPresence,
    ...
    media =
      inputs.flash_feedback > 0.001
        ? Math.abs(0.0 - inputs.flash_feedback)
        : inputs.mosh_on > 0.5
        ? inputs.drum_hits > 0.5
          ? syn_BassHits > 0.5
            ? 1.0
            : 1.0 - syn_Level * 0.125
          : inputs.syn_Presence * (1.2 - inputs.syn_BassHits * 0.25)
        : inputs._show_feedback;
    uniforms.show_media =
      alt_fb > 0.5 || (alt_auto > 0.5 && mosh_on < 0.5)
        ? 0.05 * syn_Hits
        : 1.0 - (media > 1.0 ? 1.0 : media);
