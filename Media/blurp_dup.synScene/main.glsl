...
float smoothTime = (smooth_basstime * 0.75 + TIME * 0.125 + syn_Time * 0.55)*0.45;
float smoothTimeB = (smooth_hightime * 0.75 + TIME * 0.125 + syn_Time * 0.55)*0.45;
float smoothTimeC = (smooth_midtime * 0.75 + TIME * 0.125 + syn_Time * 0.25)*0.45;
...
vec4 composite()
{
  ...
  vec2 grad3B = gradient(sampler_blur3, uv, pixelSize * 4., vec4(0,1,0,0) );
  ...
  if (auto_circling>0.5){
    light_angle_mod = vec2(0.5+0.5*sin(smoothTimeB), 0.5+0.5*cos(smoothTimeB));
  }
  ...
  colOut += vec4(flashGradient, 0.0)*_pulse(length(_uvc)+grad1.r, fract(smoothTimeB*0.2)*2.0-0.5, 0.5); 
  ...
  colOut += pow(syn_HighHits*0.4, 2.0)*lightningCol*_pulse(grad3.r*3.0, fract(TIME*0.3)*1.0-0.25, 0.2);
