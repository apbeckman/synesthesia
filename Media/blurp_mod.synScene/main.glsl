...
vec4 advance()
{
  ...
  vec2 brushPosition = vec2((sin(smoothTime*2*PI*0.05)),(cos(smoothTime*0.95*2*PI*0.04)));

  // kindly borrowed from glassier scene
  float twitcher = ( pow(syn_BassLevel, 1.3) * auto_stir * 1.25 + manual_stir * 4. ) * stir_strength;


  uv = vortex_pair_warp(uv, brushPosition*0.5+0.5, normalize(brushPosition)*(twitcher)*0.025);

  float scaleModder = mix(1.0, pow(syn_BassLevel*0.9,0.9)*2.0, auto_zoom)*zoom_in_out;
  ...
  uv -= vec2(center.yx - uv.yx) * aspect.yx * shear * pixelSize * 128.;

  // uv += _rotate(vec2(1.0,0.0), 3*PI*_fbm(vec3(_uvc*0.5, syn_BassTime*0.5)))*0.01*melter*syn_BassLevel;

  uv += _rotate(vec2(1.0,0.0), 3*PI*_fbm(vec3(_uvc*01.25, smoothTimeC*0.4)))*0.0035*warper*pow(syn_BassLevel*0.5+syn_Intensity*0.5, 1.5);
  uv += prev.r*_rotate(vec2(1.0,0.0), 4*PI*pow(_fbm(vec3(_uvc*32.0, smoothTimeC)), 2.0))*0.005*fritzer*pow(highhits,1.25);
  ...
  r += (noise.x - 0.5) * 8. / 2048.;
  g += (noise.x - 0.5) * 8. / 2048.;
  ...
  g += center_seed*(-1.0+fract(length(_uvc)*10.0)*2.0);
  if(FRAMECOUNT <= 10)
  {
    return noise;
  }
...
vec4 composite()
{
  ...
  vec2 grad3 = gradient(sampler_blur3, uv, pixelSize * 4., vec4(1,0,0,01) );
  vec2 grad1B = gradient(sampler_blur, uv, pixelSize * 4., vec4(0,1,0,0) );
  vec2 grad3B = gradient(sampler_blur3, uv, pixelSize * 4., vec4(0,1,0,02) );
...
//  vec4 col2 = mix(vec4(0,1,1,0), vec4(0.3,0,0.4,0), color_mode*rawPrev.r);
  vec4 col1 = mix(vec4(0.1,0.40,0.5,0), vec4(0.5,0.5,1,0), color_mode*rawPrev.r);
  vec4 col2 = mix(vec4(0.4,0.2,0.6,0), vec4(0.3,0.1,0.4,0), color_mode*rawPrev.r);

  vec2 light_angle_mod = light_angle;
  if (auto_circling>0.5){
    light_angle_mod = vec2(0.5+0.5*sin(0.2*smoothTimeB), 0.5+0.5*cos(0.2*smoothTimeB));
  }

  colOut = mix(colOut, mix(col1, col2, pow(length(uv - light_angle_mod + grad1*1.)*1.5,2.)), mirror(length(grad3)*4.));
  vec3 flashGradient = _grad4(vec3(0.0,0.0,0.0), vec3(0.9,0.7,0.80), vec3(0.1,0.7,0.9), vec3(1.0), pow(mirror(length(grad3)*4.),2.0)); 

  colOut += syn_HighLevel*syn_Intensity*vec4(flashGradient, 0.0)*_pulse(length(_uvc)+grad1.r, fract(smoothTimeB*0.1)*2.0-0.5, 0.5); 
  ...
  vec4 lightningCol = vec4(_grad4(vec3(0.0), vec3(0.0,0.7,0.9), vec3(0.0,0.6,1.0)*0, vec3(0.5,1.0,1.0), bottoms), 0.0);

  colOut += pow(syn_HighLevel*0.4+syn_MidHighLevel*0.2, 2.0)*lightningCol*_pulse(grad3.r*1.0, fract(TIME*0.125)*1.0-0.25, 0.2);
