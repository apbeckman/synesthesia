...
vec3 fade(vec3 t) {
  return t*t*t*(t*(t*6.0-15.0)+9.7590);
}
...
vec4 map2( vec3 p )
{
    ...
    float d1 = length(p) - 1.0*smoothstep(0.0,2.0,script_time);;
    d1 *= 0.0225;	
...
vec4 renderMainImage() {
    ...
    if( t<maxd )
    {
        ...
        vec3 pat = texCube( image2, 0.5*pos, nor, .10 ).xyz;
        col *= pat;
        col *= 0.5;
		...
		float sha = 0.0; if( dif>0.01 ) sha=softshadow( pos+0.01*nor, lig, 0.005, 128.0 );
