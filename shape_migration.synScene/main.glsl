...
float map(vec3 p){
    ...
    h.z = _smin(h.z, -h.z, 0.09);
    h1.z = _smin(h.z, -h.z, 0.09);
    vec3 c = h;
    c.xy = abs(c.xy);
    float cMod = 9.0;
    c.z += bass_time*2.5;
    c.z = mod(c.z, cMod);
    c.z -= cMod*0.5;
    c.yz = _rotate(c.yz, TIME*0.2);
    // h.x = _smin(h.x, -h.x, .1);
    c.xy = _rotate(c.xy, TIME*0.2);
    ...
    float mixed_shape = mix(first_part, tet, .35*cos(TIME*.5));    
    vec3 te = h;
    te.xy = min(te.xy + .9, -te.xy + .9);
    
    te.yz = _rotate(te.yz, PI*.5);
    float tendril = cylinderSDF(te,vec2(0.14, 1.));
    float bF = boxFrameSDF(h, bfSize, bfThiccness);
    bF = mix(bF, tendril, pow(box_tendril_mix, 4.0));
    result = opUnion(bF, mixed_shape, .05);
...
vec4 renderMainImage() {
    ...
    vec3 bg = mix(vec3(.005, .00125, .0061).zyx*(1.0+.1*pow(syn_HighLevel*0.5+syn_MidHighLevel*0.5, 2.0)), 
    vec3(.3+.1*_nsin(smoothTimeB*0.02), .17, .43+_ncos(smoothTimeB)*0.05).zyx*(1.0+.40*pow(syn_HighLevel*0.5+syn_MidHighLevel*0.5, 2.0)),
    length(-rd.xy*.35 +abs( _rotate(vec2(_noise(_uvc.xy*4.+.01*TIME)), TIME*0.5) )) + .35);
    sceneCol = sqrt(sceneCol * sceneCol);
