...
float dodec(in vec3 z) {
  ...
  float dmin=dot(z-vec3(size+SIZE,0.,0.),plnormal);
        
  dmin = abs(dmin) - width*7.5*(0.55 + 0.45*sin(10.0*length(p) - 0.5*p.y + smoothTimeC/15.0));
...
float df(vec2 p) {
  float d = 100000.0;
  float off = 0.30  + 0.25*(0.5 + 0.5*sin(smoothTimeC/11.0));
  for (int i = 0; i < rep; ++i) {
    vec2 ip = p;
    ...
    float cd = length(cp - vec2(0.5*sin(smoothTime/8.0), 0.0)) - 0.125*size;
    cd = abs(cd) - width*0.5;
    d = pmin(d, dd, 0.05);
    d = pmin(d, cd, 0.025);
  }
...
vec4 renderMainImage() {
  ...
  vec3 rgb = 0.5 + 0.5*vec3(sin(TAU*vec3(50.0, 49.0, 48.0)*(d - 0.050) + smoothTimeB/6.0));

  col += baseCol.xyz*pow(rgb, vec3(8.0, 9.0, 7.0)); 
  col *= 1.0 - tanh(0.05+length(8.0*d));

  float phase = TAU/4.0*(-length(p) - 0.5*p.y) + smoothTime/2.5;
