...
float tickTime(float t){ return t*1.5 + tick(t, 2.)*.5; }
...
void cam(inout vec3 p, float tm, float tTime) {
    ...
    #ifdef CAM_SWING
  	p.xz *= r2(sin(tTime*0.15)*.32);
  	p.xy *= r2(sin(tTime*.05)*1.25);
    #endif
...
vec4 renderMainImage() {
    ...
    float tm = (TIME+(smooth_basstime*0.25))*0.75;
    ...
    for(int j = 0; j<sampleNum; j++) {
        ...
        for(int i = 0; i<3; i++) {
            ...
            vec3 tx = texture(image48, fract(tnuv - .5 - vec2(smooth_hightime/(sc)/1.2, 0))).xyz; tx *= tx;
            tx = mix(tx, vec3(dot(tx, vec3(.299, .587, .114))), .75);
            tx = smoothstep(.1, .55, tx);

            vec3 tx2 = texture(image3, fract(tnuv2 - .5 - vec2(smooth_hightime/(sc)/1.2, 0))).xyz; tx2 *= tx2;
