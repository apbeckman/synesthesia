...
vec4 renderMainImage() {
    ...
    pos.z = (smooth_basstime*0.25)*FWD_SPEED;
    
    #ifdef	LOOKING_AROUND
    dir.zy *= mat2(cos(smooth_hightime*.25),sin(t*.5),-sin(t*.5),cos(t*.5) );
...
float	de_0(vec3 p)
{
    ...
    pr.y *= sin(script_time*.666     +p.z+p.y-p.x);
    pr.x *= sin(script_time*.666+2.09+p.z+p.y-p.x);
    pr.z *= sin(script_time*.666+4.18+p.z+p.y-p.x);
...
float	de_1(vec3 p) // cylinder
{
    ...
    q.y = rot(q.xy, vec2(-1.+sin(script_time)*2., 0.)).x;

	mind = length(q) - 4.5;
    #ifdef	BAD_TRIP
    #undef	CAM_PATH
    mind -= (sin(smooth_hightime*.75) );
	#endif
