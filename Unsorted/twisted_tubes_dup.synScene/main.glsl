...
vec4 renderMainImage() {
    ...
    vec3 u = vec3(2.*fragCoord - RENDERSIZE.xy, RENDERSIZE.y); 

    u = normalize(vec3(u.xy, sqrt(max(u.z*u.z*5.0 - dot(u.xy, u.xy)*.2, 0.))));
  ...
  //  vec2 m = sin(vec2(1.57079632, 0) + bass_time/16.);
    ...
    float sh = softShadow(sp + sn*.005, lp, 16. + 4); // Soft shadows.
    float ao = calculateAO(sp, sn)*.75 + .25 - .25; // Self shadows. Not too much.
