...
function update(dt) {
  ...
  bassT = bassT + Math.pow(Math.pow(inputs.syn_BassLevel*0.3, 2.0)+Math.pow(inputs.syn_MidPresence*0.2, 2.0)+Math.pow(inputs.syn_Level*0.15, 2.0)+Math.pow(inputs.syn_BassPresence*0.15, 2.0),2.0)+0.0125;
  uniforms.script_bass_time = bassT;

  midT = midT + Math.pow(inputs.syn_MidLevel*0.35+inputs.syn_MidHighLevel*0.15,2.0);
  uniforms.script_bass_time = bassT;

  highT = highT + Math.pow(Math.pow(inputs.syn_MidHighLevel*0.3, 2.0)+Math.pow(inputs.syn_HighLevel*0.3, 2.0)+Math.pow(inputs.syn_MidLevel*0.2, 2.0)+Math.pow(inputs.syn_MidPresence*0.2, 2.0),2.0);
  ...
  uniforms.smoothTime = (uniforms.smooth_basstime * 0.75 + inputs.TIME * 0.125 + inputs.syn_Time * 0.15);
  uniforms.smoothTimeB = (uniforms.smooth_hightime * 0.75 + inputs.TIME * 0.125 + inputs.syn_Time * 0.15);
  uniforms.smoothTimeC = (uniforms.smooth_midtime * 0.75 + inputs.TIME * 0.125 + inputs.syn_Time * 0.15);
