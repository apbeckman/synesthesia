...
vec3 surface (vec2 uv) {
    ...
    for (int i = 0; i < 15; i++) {
		uv += 0.01*(2.+1.5*sin(.1*smoothTimeC))*ff(uv);
    }
...
vec3 stars (vec2 v) {
	return vec3(pow(1.35*no(0.1*smoothTimeB+5.*mouse+3.*v/dot(v,v)),7.));
}
vec4 renderMainImage() {
	vec4 fragColor = vec4(0.0);
	vec2 U = _xy;

    vec2 uv = (2.*U-RENDERSIZE.xy)/RENDERSIZE.y;
   	float q = 1.2+0.1*min(17.,smoothTime);
    vec3 p = vec3(0,0,-q);
    vec3 d = normalize(vec3(uv,4.));
    mouse = _mouse.xy/RENDERSIZE.xy;
