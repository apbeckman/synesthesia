...
vec4 renderMainImage() {
  ...
  vec2 uv = vec2(fragCoord.x / RENDERSIZE.x, fragCoord.y / RENDERSIZE.y);
  uv -= 0.5;
  ...
  for(float j=0.0; j<steps; ++j) {
    ...
    for(float i=0.0; i<3.0; ++i) {
      ...
      addcol *= smoothstep(0.5*curve(smoothTimeC+id.y*0.01+id.x*0.03, 0.3),0.0,hash2(id));
      ...
      addcol += vec3(0.7,0.5,1.2)*step(p2.y,-0.9)*max(0.0,curve(smoothTimeC,0.2)*2.0-1.0)*step(hash2(id+.7),0.2);
      col += addcol * alpha;
