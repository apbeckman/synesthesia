...
vec4 formula (vec4 p) { // TIME VARIABLES HERE
	p.y-=smoothTimeC*.25;
...
vec2 de(vec3 pos) {
	float aa=smoothstep(0.,1.,clamp(cos(t-pos.y*.4)*1.5,0.,1.))*3.14159; //more time variable
    cc=cos(aa);ss=sin(aa);
    float hid=0.;
	vec3 tpos=pos;
    ...
    fl-=(length(p.xz)*.005+length(sin(pos*3.+t*5.))*.15); //TIME
    fl*=.9;
...
vec4 renderMainImage() {
	...
	vec2 uv = fragCoord.xy / RENDERSIZE.xy*2.-1.;
    vec2 uv2=uv;
