...
#define time (smoothTime*.25)
#define time2 (smoothTimeC*.25)
...
{
    float d = 1.0;
    d *= distance(p, vec3(cos(time2*.14)+sin(time2*0.2),0.3,2.0+cos(time2*0.25)*0.5) );
    d *= distance(p,vec3(-cos(time2*0.7),0.3,2.0+sin(time2*0.25)));
    d *= distance(p,vec3(-sin(time2*0.2)*0.5,sin(time2*0.2),2.0));
...
vec4 renderMainImage() {
	...
	vec2 v = -1.0+2.0*q;
	v.x *= RENDERSIZE.x/RENDERSIZE.y*.5+.5;
	
	vec3 o = vec3(v.x,v.y,0.0);
    vec3 d = normalize(vec3(v.x+cos(smoothTime*0.2)*.1,v.y,1.0))/64.0;
