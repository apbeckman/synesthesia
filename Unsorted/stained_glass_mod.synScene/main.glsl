...
vec4 renderPass()
{
    ...
    float time = smoothTimeB * PI * 0.0125;
    ...
    float value2 = 0.;
    float value3 = 0.;
    vec3 value4 = vec3(1.0,0.0,syn_BassLevel+syn_MidLevel);
    ...
    for(int j = 0; j < iters; j++){
      ...
      uv = _rotate(uv, 100.0*j+spin_time)*(1.0+length(uv)*0.0005*distortion);
      uv += move_xy;
      uv2 = _rotate(uv2, 100.0*j+spin_time)*(1.0+length(uv2)*0.0005*distortion);
      uv2 += move_xy*0.1;
      uv.y += img*10.0;
      ...
      for(int i = 0; i < points ; i++){
        ...
        float zoom = mod(bass_time+j,iters);
        // zoom -= img.r;
