...
float growthFactor = pow((syn_BassLevel*0.5)+(syn_MidLevel*0.375)+(syn_Intensity*0.125), 2.0);
...
vec2 hash22(vec2 p) { 
    ...
    p = mod(p, 64.);///vec2(RENDERSIZE.y/RENDERSIZE.x, 1)
...
//fragCoord.xy = _rotate(fragCoord+Zoom*_uvc, syn_BassLevel*0.001)+_uvc;
    ...
    */
    ...
    if(FRAMECOUNT<=10 || Reset > 0. ||abs(rdVal.w - RENDERSIZE.y)>.001 )  {
        ...
        fragColor.xy =  vec2(1, 0) +  (vec2(Voronoi(p*64.), Voronoi(p*64. + vec2(2.93, 1.37))) - .5)*0.1+vec2(-1, 1)*(sTexture(syn_UserImage, p).xy*(1.+basshits) - .5);
...
float bumpFunc(vec2 p){ 
    ...
    difPat = smoothstep(0., 1.5, difPat - .165);
    ...
    gs += smoothstep(0., .105, ggs - .525)*.05 - .025;
...
vec4 renderMainImage() {
    ...
    uv+=0.125*sin(noise(_uvc*PI+uv))*PI*(Warp);
    ...
    rd = normalize(vec3(rd.xy, sqrt(rd.z*rd.z - dot(rd.xy, rd.xy)*.025))); // Very subtle lens warping.
 ...
 //  ro.z -= (Fov-0.55);
    ...
    vec3 sn = normalize(vec3(.025, .07, -1)); 
    ...
    // sp.xy -= vec2(_noise(vec2(-5*sin(smoothTime*0.0005+TIME*0.01), -5.5*cos(smoothTime*0.0005+TIME*0.01))));
    ...
    float atten = min(1./(1. + lDist*0.125 + lDist*lDist*0.0125), 1.);
    ...
    texCol = mix(vec3(2), texCol, mainPat);
    ...
    difPat2 =-4*(1.0*DifPatCol)* smoothstep(0., .05, -difPat2 + .5);
    ...
    // or something to that effect.
     ...
     texCol = mix(vec3(.20, .10,.2)*(difPat2*.5 + .75)*sp.x, vec3(.1)*2.5, mainPat)*texCol*(1.0+syn_HighLevel*syn_Intensity);
    ...
    vec3 col = (texCol*(diff*2. + 0.5) + (texCol*.5 + 1.5)*vec3(.5, .5, .5)*spec)*atten;
