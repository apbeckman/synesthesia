...
vec2 map(vec3 p) {
    ...
    #else 
    p += sin(p * 08. + time * vec3(1,3,2) * PI * 2. + vec3(0,1.5,3)) * .1;
    ...
    #ifdef ALTERNATIVE
    float e = .1;
    p.x += sin(p.x * 04. + (time * .5 + .0) * PI * 3.) * e;
    p.z += sin(p.z * 04. + (time * -.25 + sign(p.x) * .25 + .25) * PI * 2.) * e;
    p.y += sin(p.y * 04. + (time * -.75 + sign(p.z) * .25) * PI * 2.) * e;
...
vec4 renderMainImage() {
    ...
    time = mod(smoothTime*0.1 / duration + .1, 1.);
    //time= 0.;
    ...
    #else
    float h = hex(uv.yx * 1.08 + time * vec2(.2,.172) * 2.);
    #endif
    
    h -= (.03+syn_BassLevel*0.02);
    ...
    vec3 bgCol = BGCOL * .018 * .0125*(syn_HighLevel*syn_Intensity*(1.0+syn_Intensity));
    vec3 bgCol2 = (bgCol*10) * (.3+syn_Level+syn_MidHighLevel*2.*syn_Intensity+syn_HighLevel)*(1.0+syn_HighLevel+syn_Intensity);
	...
	camOrigin = vec3(0,0,9.5 * fl);
   	camDir = normalize(vec3(_uvc/PI, -fl));
    ...
    for (float disperse = 0.; disperse < MAX_DISPERSE; disperse++) {
		...
		float rand = texture(image14854, (fragCoord + floor(smoothTimeC * 60.) * 10.) / RENDERSIZE.xy).r;
