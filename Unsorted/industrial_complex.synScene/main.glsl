...
float lattice(vec3 p){
    ...
    rail = min(rail, posts);
    float railB = max(rail, -hole2);
    railB = max(railB, -hole3);

    rail = mix(rail, railB, pow(Escape,2.5));
...
float trace(vec3 ro, vec3 rd){
    ...
    float t = 0., d;
    int numSteps = int(80 - 70*pow(Ray_Steps,.2));
...
float bumpFunction(in vec3 p){
    ...
    if (Camera_Wrap > 0.5) { p.xy -= camPath(p.z).xy; }
...
vec3 nr(vec3 p, inout float edge, float t){
    ...
    vec2 e = vec2(2./RENDERSIZE.y, 0); // Larger epsilon for greater sample spread, thus thicker edges.
...
float softShadow(vec3 ro, vec3 lp, float k){
    ...
    const int maxIterationsShad = 100;
...
vec3 envMap(vec3 rd, vec3 n){
    ...
    vec3 col = tex3D(image4, rd, n);
    col = smoothstep(.15, .5, col);
    if (Warmth > 0.5) { col *= vec3(1.35, 1, .65); }
...
vec4 renderMainImage() {
	...
	// Camera Setup.
    float speed = Fly_Speed; // 3.
    float camTime = syn_Time * .5;
    vec3 ro = camPath(camTime*speed); // Camera position, doubling as the ray origin.
    vec3 lk = camPath(camTime*speed + .5);  // "Look At" position.

    lk.xy += Look_XY * Cam_Reset;

    vec3 lp = camPath(camTime*speed + 5.5); // Light position, somewhere near the moving camera.
	...
	//vec3 lp = vec3(0, 0, TIME*speed) + vec3(4, .5, 3.5);

    if (Camera_Wrap > 0.5) { lp.x += 8.; }

    // Using the above to produce the unit ray-direction vector.
    float FOV = 3.14159/(3.-FOV); ///3. FOV - Field of view.
    ...
    float edge2 = sin(syn_BassTime);
    if (Textures > 0.5) sn = doBumpMap(sp + texture_react*2.5*sin(syn_Time*.25), sn, .15/(1. + t/FAR), edge2);
    ...
    sn = doBumpMap(image4, tsp*tSize, sn, .01/(1. + t/FAR));
    ...
    vec3 tx = tex3D(image3, tsp*tSize, sn);
    tx = smoothstep(0.05, .5, tx)*1.5; // Giving it a bit more brightness and contrast.

    vec3 warm = vec3(1.35, 1, .65);
    tx *= (warm + Warmth*warm) / warm;



    if (Textures > 0.5) if(bObjID > .5 && bObjID < 1.5) tx *= vec3(1.3, .65, .35);//vec3(1.5, .85, .25);
    ...
    if(sp.y<-3.49 && sp.y>-3.51) {
        ...
        //dif = (sqrt(dif));
        if (Textures > 0.5) tx *= tiles(sp.xz);
        tx *= vec3(1.3, 1, .7);
    }
    ...
    if (Warmth > 0.5) {
      ...
      bg = blackbodyPalette(noise*(1+syn_BassHits*.1));
    } else {
      float noise = (fBm(sp - TIME*2.)*.15 + .9)*(rd.y*.5 + .5);
      bg = mix(vec3(.3, .4, .7), vec3(.7, .9, 1), noise);
    }
