...
float d2 = dist(RENDERSIZE.xy*0.5,_xy.xy)*0.000025;

vec4 renderMain(void) {
	vec2 position = _uv;
    float d3 = _noise(d2+TIME);
	position += _muv;
	vec3 col = vec3(0.);
	vec4 newcol = vec4(1.);
    ...
    if (PASSINDEX == 0) {
    	...
    	if (warp > 0.2 || (warp_beat > 0.5 && syn_BPMConfidence > 0.5)) {
            ...
            lastuv.y += (_noise(_uvc * 1. * warp_noise_scale + TIME - 1000.) * 0.01 - 0.005) * warp_noise;
            vec2 hitswarp = vec2(
    	...
    	if (_click.x > 0.5) {
            vec2 m = _muvc;
            // m.x = mix(m.x, -abs(drawuv.x), mirror);
            vec2 drawuv = (_uvc - m) - window_size;
            // drawuv.x = mix(drawuv.x, (abs(_uvc.x) + m.x) - window_size, mirror);
            // drawuv += (_uvc + m) - window_size;
