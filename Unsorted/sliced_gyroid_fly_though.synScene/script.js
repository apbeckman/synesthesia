...
function update(dt) {
  ...
  try {
    ...
    highTimevar.updateTime(0.125, Math.pow((inputs.syn_HighLevel+inputs.syn_MidHighLevel*1.5+inputs.syn_MidLevel*0.75+inputs.syn_Intensity*0.5), 2.0)*(inputs.light_time), dt);

    spinTimevar.updateTime(.2,  Math.pow(0.5+inputs.syn_BassLevel*0.75+inputs.syn_MidLevel*0.75+syn_Intensity,2.0)*(-inputs.Rotate), dt);
    ...
    uniforms.high_time = highTimevar.time;
