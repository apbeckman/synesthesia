...
vec4 renderPassA() {
    ...
    float du = 0.1;
    float stSz = R.y*0.004*(1. + sin(TIME + dot(uv,uv)*10.));
    ...
    vec3 prevFr = A(U + dF*stSz*2.*(.2 + sin(TIME)*0.05 + dot(dF,dF) )).xyz;

    vec3 col = mix(vec3(0.96,0.9,0.8 - dot(uv,uv)*0.1 ), prevFr,0.99 + sin(TIME*4.)*0.009);
    
    float ddf =  dot(dF,dF)*1.;
    float cenv = max(sin(TIME*0.6),0.);
    cenv = smoothstep(0.,1.,cenv);
    for(float i = 0.; i < 130.; i++){
        vec2 p = uv + vec2(sin(i)*0.25*cenv,0);
        p *= rot(i*10. + (TIME +sin(TIME + i*2.1)*1.)*0.4*sin(i));
        //p.y += sin(i*240.)*5.;
        
        p = vec2(length(p),atan(p.xy));
        
        float env = mod(sin(i)*10.2 + TIME*0.1,1.)*1. + sin(TIME)*0.15;
        p.x -= env;
        ...
        vec3 c = pal(vec3(1.,1.,0.4),vec3(0.4,0.8,0.9),vec3(3,2,1),1.,i*3. + TIME + p.x*3. - ddf + p.x*47. + 0.*smoothstep(0.03,0.1,d));
