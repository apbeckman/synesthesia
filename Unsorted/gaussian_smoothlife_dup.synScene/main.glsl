...
float shock = shock_forward - shock_back;

			//******** BuffA Code Begins ********

float sHighHits = syn_HighHits * audio_reactivity;
float sBassHits = low * audio_reactivity;
...
// float alpha_n = 0.02; 
// float alpha_m = 0.11;
vec4 media = texture(media_pass_fx, _uv);
// vec4 media_edges = texture(media_pass_fx, _uv);
bool reset() {
...
float s(float n, float m) {
  return sigma2(n, sigma_m(b1, s1, m, alpha_m), sigma_m(b2, s2, m, alpha_m), alpha_n);
}
...
vec4 renderPassA() {
  ...
  vec2 fragCoord = _xy;
  fragCoord += _uvc * Zoom * (1.0 + syn_BassLevel + shock * 0.2);
  fragCoord += abs(d2) * _uvc * (Fisheye - shock * PI) * (1.0 + 2.0 * low + syn_Intensity * 1.5);
  ...
  float ringval = length(_noise(TIME*0.1+sin(_uvc))*rings);
  ringval = mix((ringval), length(_uvc)*rings, .25);
  ringval = mix((ringval), length(_uvc)*rings, 0.75-rings*0.125);
  fragColor -= cycling_rings * fragColor * sin(ringval * 6.0 + .25*smoothTime) * 0.1;

  float lum = dot((media.rgb * 1.1), vec3(1.0)) / 3.0;

  fragColor.r = max(fragColor.r, pow(lum, contrast * contrast));
  if(_mouse.z > 0. && _noise(_uv)+length(fragCoord - _mouse.xy) < paintsize*5 && length(fragCoord - _mouse.xy) > (paintsize*5 - (paintsize*paintfull))) {
    fragColor.r = .99;
  }
...
vec4 renderPassB() {
  ...
  if(_mouse.z > 0. && _noise(_uv)+length(fragCoord - _mouse.xy) < paintsize*5 && length(fragCoord - _mouse.xy) > (paintsize*4.9)) {
    fragColor.r = .99;
  }
...
vec4 mediaPassFX() {
  vec4 mediaFX = mix(texture(media_pass, _uv), _edgeDetectSobel(media_pass, _uv), media_edge_mix * 0.5);
  return mediaFX;
}
...
vec4 renderMainImage() {
  ...
  float colSel = mix(mix(blur, semiBlur, 0.25 + 0.75*syn_Presence * audio_reactivity), hard, 0.5 * syn_MidHighLevel * audio_reactivity);
  ...
  vec3 palCol = vec3(0.0, 0.0, blur) * (0.2+0.8*syn_BassLevel) + colSel * colorPaletteChooser(color_regime, colSel);

  vec3 normCol = col.x * vec3(1.0) + col.y * vec3(1, 0., 0.5) + col.z * vec3(0.25, 0.5, 0.75);
  normCol = _grayscale(normCol, 0.1 + greyscale * 0.7);
