...
SmoothCounter.prototype.update = function (dt, newCount, speed) {
  this.currentValue =
    this.currentValue + (newCount - this.currentValue) * speed;
};
...
function update(dt) {
  ...
  try {
    ...
    noiseTimevar.updateTime(
      1.0,
      _noise(
        Math.pow(
          0.5 +
            inputs.syn_BassLevel * 2.0 +
            inputs.syn_MidLevel * 1.0 +
            syn_Intensity * 1.0
        ,
        2.0)
      )*3.0,
    ...
    midTimevar.updateTime(
      0.125,
      Math.pow(
        inputs.syn_BassLevel * 0.785 +
          inputs.syn_MidHighLevel * 0.9 +
          inputs.syn_MidLevel * 1.5 +
          inputs.syn_Intensity * 0.5,
        2.0
      ) * inputs.mid_time,
      dt
    );
    spinTimevar.updateTime(
      0.1,
      Math.pow(
        0.5 +
          inputs.syn_BassLevel * 0.75 +
          inputs.syn_MidLevel * 0.75 +
          syn_Intensity,
        2.0
      ) * inputs.Rotate,
    ...
    uniforms.noise_time = noiseTimevar.time;
    uniforms.bass_time = bassTimevar.time;
    uniforms.mid_time = midTimevar.time;
    uniforms.spin_time = spinTimevar.time;

    uniforms.highhits = Math.pow(
      (inputs.syn_HighLevel * 0.5 +
        inputs.syn_Hits * 0.125 +
        inputs.syn_HighHits * 0.375) *
        inputs.syn_Intensity,
      2.0
    );
    uniforms.basshits = Math.pow(
      (inputs.syn_BassLevel * 0.5 +
        inputs.syn_Level * 0.125 +
        inputs.syn_BassHits * 0.375) *
        inputs.syn_Intensity,
      2.0
    );
    uniforms.midhits = Math.pow(
      (inputs.syn_MidLevel * 0.5 +
        inputs.syn_Level * 0.125 +
        inputs.syn_MidHits * 0.375) *
        inputs.syn_Intensity*inputs.syn_Presence,
      2.0
    );
    uniforms.hits = Math.pow(
      (inputs.syn_HighLevel * 0.5 +
        inputs.syn_Level * 0.125 +
        inputs.syn_Hits * 0.375) *
        inputs.syn_Intensity,
      2.0
    );
    uniforms.high = Math.pow(
      (inputs.syn_HighLevel * 0.65 +
        inputs.syn_MidHighLevel * 0.35) *
        inputs.syn_Intensity*inputs.syn_Presence,
      2.0
    );
    uniforms.low = Math.pow(
      (inputs.syn_BassLevel * 0.65 +
        inputs.syn_MidLevel * 0.35) *
        inputs.syn_Intensity,
      2.0
    );
    ...
    highT =
      highT +
      Math.pow(
        inputs.syn_MidHighLevel * 0.45 +
          inputs.syn_HighLevel * 0.35 +
          syn_Intensity * 0.2,
        2.0
      );
    ...
    uniforms.smoothTime =
      uniforms.smooth_basstime * 0.75 +
      inputs.TIME * 0.125 +
      inputs.syn_Time * 0.2;
    uniforms.smoothTimeB =
      uniforms.smooth_hightime * 0.75 +
      inputs.TIME * 0.125 +
      inputs.syn_Time * 0.2;
    uniforms.smoothTimeC =
      uniforms.smooth_midtime * 0.75 +
      inputs.TIME * 0.125 +
      inputs.syn_Time * 0.2;
