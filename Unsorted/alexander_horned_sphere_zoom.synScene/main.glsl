...
vec4 renderPassA() {
    ...
    float aspectWoverH = RENDERSIZE.x/RENDERSIZE.y;
    ...
    if (didHit > 0.5) {
        ...
        float ctt = fract(-0.2*TIME + smoothBranchDepth/4.0);
        vec3 matColor = 0.5 + 0.5*vec3(
            cos(TWO_PI * ct),
            cos(TWO_PI * (ct - 1.0/3.0)),
            cos(TWO_PI * (ct - 2.0/3.0))
        );
        float pulse = 1.0 - step(0.25+0.25*sin(0.1*smoothTime), abs(ctt-0.5));
