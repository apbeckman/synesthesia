...
vec3 texFaces(in vec3 p, in vec3 n){
    ...
    vec2 sc = cos(mod(hexTruchet(p.xy)*6.2831*vec2(2, 4), 6.2831));
    sc = _rotate(sc, smoothTimeC*0.125);
    n *= clamp(dot(sc, vec2(.6))+.5, 0., .45*(1.+0.75*pow(syn_HighLevel, 2.)))*.95 + .05;
    return min(n, 1.);
...
vec4 renderMainImage() {
    ...
    fragCoord = _noise(_xy);
    vec3 threeWaySin = vec3(_nsin(smoothTimeB*0.125)*0.25, _ncos(smoothTimeC*0.125)*0.125, -_nsin(smoothTimeC*0.125)*0.25);
    ...
    vec3 rd = vec3(2.*fragCoord +_uvc*PI*Fov - RENDERSIZE.xy, RENDERSIZE.y);
    float scaledFov = _map(Fov, -2., 1.,  0.,  1.);
    // Barrel distortion;
    rd = normalize(vec3(rd.xy*Fov+Fov*_uvc, sqrt(max(rd.z*rd.z - dot(rd.xy, rd.xy)*.2, 0.))));
    ...
    rd.xz = _rotate(rd.xz, -PI*lookXY.x+PI*_uvc.x*Flip*Fov+0.45*_fbm(cos(0.025*TIME)));
    rd.yz = _rotate(rd.yz, -PI*lookXY.y+0.45*_fbm(sin(0.025*TIME)));
...
//    vec3 ro = vec3(0, 0, TIME);
    ...
    // vec3 ro = vec3(0, bass_time/2. + .5, bass_time/2.);
    
    vec3 lp = ro + vec3(.42, .8, .3); // Light, near the ray origin.
    ...
    vec3 bg = mix(vec3(.65, .7, .9).zyx, vec3(1, .37, .3).zyx, -rd.y*.35 + .35)*(1.0+bg_flash*pow(0.5*syn_HighLevel*0.5+0.5*syn_MidHighLevel+ syn_Intensity*0.25, 2.));
    bg += threeWaySin;
