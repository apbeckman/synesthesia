...
float map(in vec3 p){
    ...
    float tx = (-textureLod(image47, p.xz/22. + p.xy/80., 0.).x-0.4) * 0.95;
    
    vec3 q = sin(p.zyx*.2 - (0.5+tx)*0.5 + TIME*0.13)*sminP(5.0, pow((0.5-tx)*5.5, 0.22), 3.);
...
vec4 renderMainImage() {
	...
	vec3 lookAt = vec3(0, 0, 18.+TIME*1.9);
    ...
    if (t < FAR){
        ...
        sn = doBumpMap(image6, sp*tSize1, sn, .007/(1. + t/FAR));
