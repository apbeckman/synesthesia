...
float brightness(vec2 uv, vec2 id) {
    ...
    float twinkle = sin((smoothTimeB*0.275)+n*6.28)*.5 +.5;
...
vec4 ScaleCol(vec2 uv, vec2 p, vec4 i) {
    ...
    c.rgb = mix(c.rgb+highhits*0.125, col2, i.x);				// add edge highlights
...
vec4 renderMainImage() {
	...
	vec2 uv = fragCoord.xy / RENDERSIZE.xy;  // get uvs in 0 to 1 range
    ...
    vec2 uv2 = uv * SIZE*zoom;						// uv2 -2.5 - 2.5
    ...
    float pulse = _pulse(length(center.xy), smoothstep(.1, 0.2, 0.3), TIME);
   ...
   // shape = mix(center, outside, 1.);
    ...
    c.r += sin(smoothTimeC*0.05)*.3+0.05;

    c.g -= abs(sin(smoothTimeC*.0845))*.45;

    c.b += sin(smoothTimeC*0.096)*.35;

    

    float y = pow(uv.y-.5,2.)*1.5;				// vignette
