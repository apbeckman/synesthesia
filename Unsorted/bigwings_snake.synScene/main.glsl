...
vec4 ScaleCol(vec2 uv, vec2 p, vec4 i) {
    ...
    c.rgb = mix(c.rgb+highhits*0.125, col1, i.y*i.x*.5);		// add spokes
...
vec4 renderMainImage() {
    ...
    m = m*2. - 1.;
    float t = smoothTime*0.35;
    
    
    
   	uv.x += sin(smoothTime*0.1+uv.y)*.1;
   ...
   // shape = mix(center, outside, 1.);
    ...
    t*=.1;										// rainbow....
    c.r += sin(smoothTimeB*0.5)*.3+0.05;
    c.g -= abs(sin(t*1.324))*.435;
    c.b += sin(t*0.324)*.3;
    
    float y = pow(uv.y-.5,2.)*1.;				// vignette
    c *= 1.-y;
