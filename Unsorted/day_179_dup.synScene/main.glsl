...
#define T (script_time*0.125)+(TIME*0.1)+(smooth_basstime*0.051)
...
vec2 map(vec3 p){
       ...
       // p.z += T; 
        ...
        for(int i = 0; i < 4; i++){
            ...
            //q.xz *= rot(0.5);
            q.xy *= rot(0. + sin( (T + kick*pi/2.)*0.86)*0.14 );
    	...
    	glow += 8.7/(0.01 + dTrans*dTrans*200.)*pal(0.5,0.5,vec3(0.89,0.2,0.1),1.,length(p) + smooth_hightime )*pow(abs(sin(length(p)*4. + T - kick*4.)),40.);
...
vec4 renderMainImage() {
    ...
    ro.y += sin(T);
    float nF = 1.0 + nearFar;
    ro.xz += vec2(cos(T)*0.4 + nF,sin(T)*0.5-1.2)*9.;
