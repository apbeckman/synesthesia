...
{
	p.x += 0.1*sin( smoothTimeC*0.125 + 2.0*p.y ) ;
	p.y += 0.1*sin( smoothTimeC*0.125 + 2.0*p.x ) ;
	
	float a = noise(p*1.5 + sin(0.1*smoothTimeC))*6.2831;
	a -= offset;
...
vec4 renderMainImage() {
    ...
    float offset = TIME + fragCoord.x/RENDERSIZE.x;
