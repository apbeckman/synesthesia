...
vec2 path(float t) {
	float a = sin(t*.12 + 1.5)/2.0, b = sin(t*.2);
	return vec2(a*2., a*b);
}
...
vec4 renderPassA() {
	...
	vec3 c = mix(vec3(.9, .2, .4), vec3(.3, cos(smoothTimeC)*.1, .2), uv.x + ri);
	c.r *= sin(TIME);
	c += g * .015;
	fragColor = vec4(c, 1);
	return fragColor; 
...
vec4 renderMainImage() {
	...
	float s = sin(smoothTimeB*0.25)*.008;
	float t = tan(smoothTimeB*0.25)*.002;
	fragColor.r = texture(BuffA, uv + vec2(-s, s)).r;
	fragColor.g = texture(BuffA, uv + vec2(-t, t)).g;
	fragColor.b = texture(BuffA, uv + vec2(s, -s)).b;
