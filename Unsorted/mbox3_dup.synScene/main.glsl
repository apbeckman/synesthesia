...
vec4 renderMain() { 
	...
	float t = 0.;
	vec3 p;
	float t_cam = TIME/(speed*4);

    float tr = syn_Time*0.01;
	vec3 camPos = CameraPath(t_cam); 
	if ( speed == 0.0 ) 
		camPos = vec3(x,y,z) * (In_Out);
		    rotate(camPos.zx, rXY.x+cos(tr));
    rotate(camPos.zy, rXY.y+sin(tr));
    rotate(camPos.xy, rZ-sin(tr)+tr);
