...
float trap(in vec3 tp) {
	...
	return abs(min(torus(tp, vec2(epi, 0.125)), max(abs(tp.z)-0.125, abs(tp.x)-0.125)))-0.00125;
...
float map(in vec3 pm) {
	...
	for (float i = 0.0; i < 4.0+its; i++) {
		...
		m = abs(1.0-mod(m+i/(3.0+test+(0.5*(spin_time*0.5))*2.),2.0));
...
vec3 intersect(in vec3 rayOrigin, in vec3 rayDir) {
	...
	if (d < 0.0001) {
		...
		float multiplier = (1.0+(syn_HighLevel+0.75*syn_MidHighLevel)*0.5+0.5*syn_Intensity);
		col += hsv(d, 1.0,2.0*multiplier)*md*(28.0)*multiplier;
...
vec4 renderMain() { 
	...
	vec3 co = vec3(( bass_time*0.25+50.), 0, 0);
