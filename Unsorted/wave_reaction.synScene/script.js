...
function update(dt) {
  ...
  try {
    bassTimevar.updateTime(0.25, Math.pow((0.5+inputs.syn_BassLevel+inputs.syn_MidLevel*0.75+syn_Intensity*0.25), 2.0)*(inputs.distort_time+inputs.syn_Intensity), dt);

    midTimevar.updateTime(0.25, Math.pow((inputs.syn_BassLevel*0.785+inputs.syn_MidHighLevel+inputs.syn_MidLevel*1.5+syn_Intensity), 2.0)*(inputs.distort_time), dt);
