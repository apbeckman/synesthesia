...
float freqA = 0.15 + Frequency_A;
float freqB = 0.25 + Frequency_B;
float ampA = 2.4 + Amplitude_A  + sin(morph_time);
float ampB = 1.7 + Amplitude_B + cos(morph_time);
...
vec4 renderMainImage() {
	...
	// Camera Setup.
	vec3 lookAt = vec3(0.0, 0.0, fly_time);  // "Look At" position.
	vec3 camPos = lookAt + vec3(LookXY*-1, -0.1); // Camera position, doubling as the ray origin.
    ...
    float FOV = PI/(-2.5*pow(FOV,.333) + 3.); // FOV - Field of view.
	...
	if(dt<0.0025){
	    ...
	    sn = doBumpMap(image2, sp*tSize0, sn, 0.06 + Bump_Map);
    	...
    	// Curvature.
	    float crv = clamp(curve(sp, 0.35)*0.75 + 0.5, .0+syn_BassLevel*.25*React_Lights, 1.);

	    // Fresnel term. Good for giving a surface a bit of a reflective glow.
        float fre = pow( clamp(dot(sn, rd) + 1., .0, 1.), 1.);

        // Obtaining the texel color.
	    vec3 texCol = tex3D(image2, sp*tSize0, sn); // Sandstone.
      vec3 nonTexCol = hueShift(vec3(1,0,.5),2*sin(bass_time)); // Non-Textured color

      texCol = mix(nonTexCol, texCol, Texture);
	    ...
	    float shading = crv*0.9 + 0.1 - Shadows;
