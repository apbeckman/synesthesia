...
function update(dt) {
  ...
  try {
    bassTimevar.updateTime(0.5, Math.pow((Math.max(0.75, inputs.syn_BassLevel)+inputs.syn_MidLevel*0.75+syn_Intensity*0.25), 2.0)*(inputs.rate_in+inputs.syn_Intensity), dt);

    midTimevar.updateTime(0.75, Math.pow((Math.max(0.75, 0.9*inputs.syn_BassLevel)+inputs.syn_MidHighLevel+inputs.syn_MidLevel*1.5+syn_Intensity), 2.0)*(inputs.slither), dt);
