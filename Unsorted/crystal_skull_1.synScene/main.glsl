...
float marble_df(vec3 p) {  
  ...
  for (int i = 0; i < max_iter; ++i) {
    ...
    p.yz = csqr(p.yz+0.3*vec2( cos(smoothTimeC*0.01)*0.5,0.5*sin(0.01*smoothTimeC)));
    p    = p.zxy;
    res  += exp(-2. * L2(p-c));
  }
...
vec3 effect(vec2 p, vec2 q) { 
  ...
  vec3 end    = mix(0.4, 0.6, 1.)*vec3(0, 0.2, -2.0);
  float fi    = smoothstep(1.0, 10.0, smoothTime);
  vec3 ro     = mix(start, end, fi);
  ro.xz      =_rotate(ro.xz, 0.01*smoothTime);
  ro.yz      =_rotate(ro.yz, 0.001*smoothTime);
