...
float distfunc(vec3 pos)
{
	...
	return min(max(max(
	periodic(r,3.0,0.2),
	periodic(pos.z,1.0,0.7+0.15*cos(smoothTimeC/3.0))),
	periodic(a*r,3.141592*2.0/6.0*r,0.7+0.15*cos(smoothTimeC/3.0))),
	0.25);
...
void mainVR(out vec4 fragColor, in vec2 fragCoord, in vec3 pos, in vec3 dir)
{
	...
	float a=cos(smoothTimeC)*0.0*0.4;
...
vec4 renderMainImage() {
	...
	vec2 coords=(2.0*fragCoord.xy-RENDERSIZE.xy)/max(RENDERSIZE.x,RENDERSIZE.y);

	vec3 ray_dir=normalize(vec3(coords,1.0+0.0*sqrt(coords.x*coords.x+coords.y*coords.y)));
	vec3 ray_pos=vec3(0.0,-1.0,smoothTime*1.0);
