...
Scene voxelMarch(vec3 ro, vec3 rd) {
    ...
    for (int i = 0; i < 128; i++) {
        ...
        if (length(p.xy - path(p.z).xy) > 3.0) {
            ...
            float r = hash13(p);
        	float r1 = r + smoothTimeC*0.5 + p.z;
        	float r2 = r1 * 2.0 + p.y;
...
void setupCamera(vec2 fragCoord, inout vec3 ro, inout vec3 rd, inout vec3 lp) {
    ...
    vec3 lookAt = vec3(0.0, 0.0, smoothTime*1.50);
    ro = lookAt + vec3(0.0, 0.0, -1.0);
    lp = lookAt + vec3(0.0, 0.0, 16.0 + sin(smoothTime * 0.2) * 6.0);
...
vec4 renderMainImage() {
    ...
    if (scene.t > 0.0) {
        ...
        float la = mod(rp.z + smoothTimeB * 2.0, 30.0);
