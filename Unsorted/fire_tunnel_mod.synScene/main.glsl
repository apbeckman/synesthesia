...
vec4 march(vec4 f, vec3 ro, vec3 rd, float st)
{
	...
	{      
		...
		{
			w = (1.-td) * (h-s) * i/60.;
			f.rgb += texture(syn_UserImage, p.xz*0.01+p.y*0.01).rgb*0.02*(syn_Intensity*0.5+syn_HighLevel*1.5)*_pulse(fract(p.y*0.1-(smoothTimeB*0.06125)), 0.5, 0.5);
			f.rgb += getHotColor(td*i*90.)*mix(w, (0.05-w), syn_Intensity*(syn_MidLevel*1.5+syn_MidHits*0.5)*_pulse(fract(p.z*0.1-(smoothTimeB*0.125)), 0.5, 0.5));
			td += w;
		}
...
vec4 renderMainImage() {
	...
	vec2 g = _xy;
		...
		t = bass_time;
		f = vec4(syn_MidPresence*0.125+syn_Intensity*0.25,0.125,0.532,1);
