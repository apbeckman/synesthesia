...
float freqB = .25/2.75;
float ampA = 20.;
//+ Morph_Gain*sin(syn_MidHighTime*.2*Morph_Gain)*Morph_Time;
float ampB = Amplitude*4.;
...
vec2 path(in float z) {
  ...
  float morphTime = syn_BassTime * 0.1 * Morph_Time;
...
float map(in vec3 p){
    ...
    float tx = textureLod(image47, p.xz/16. + p.xy/80., 0.).x;
...
vec4 renderMainImage() {
	...
	// Camera Setup.
  float camZ = syn_Time * 2 * Fly_Speed * Reverse_Fly;
    ...
    if (t < FAR){
        ...
        sn = doBumpMap(image2, sp*tSize1, sn, .007/(1. + t/FAR));//max(1.-length(fwidth(sn)), .001)*hash(sp)/(1.+t/FAR)
        ...
        col =  smoothstep(-.5, 1., tex3D(image2, sp*tSize1, sn))*(col + .25);
