...
#define grd 0.125*vec2(e.z-w.z,n.z-s.z)
#define div 0.25*(e.x-w.x+n.y-s.y)
#define dt (FRAMECOUNT<500?1.:0.1)
#define I 5
//Dave H :
vec3 hash33(vec3 p3)
{
...
vec4 renderMainImage() {
    ...
    vec3 color = pow(vec3(.0,0,.0), vec3(2.2));    
    ...
    for( int i=0; i < iter; i++ ){
        ...
        color += m.colour * pow(max(0., (.02 - h)) * 19.5, 11.-syn_Intensity) * 50.*(1.0+syn_Intensity*syn_HighLevel);
        color += m.colour * .001 * FUDGE_FACTOR;
    }
    
    color = pow(color, vec3(1./1.8)) * 1.5;
    color = pow(color, vec3(1.5))*(1.0+syn_HighLevel*0.125);
    color *= 3.5;
