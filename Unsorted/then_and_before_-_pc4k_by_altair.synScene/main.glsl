...
vec4 renderPassA() {
	...
	{
		int spos = 0;
		float stim = TIME*1000.*441./10./(9586.*2.);
		int istim = int(stim);
	...
	if( uv.y>.11 && uv.y<.89 )
	{
		float t1 = 0., t2 = 0., seed = uv.x*uv.y+uv.y+uv.x+fract(TIME);
		vec3 ro1 = vec3(0, 0, -.1),
			rd1 = normalize(vec3((2.*fragCoord.xy-RENDERSIZE.xy)/RENDERSIZE.y/1.8, 1)),
		...
		for( int i = 0; i < QN; i++ )
		{
			...
			scol +=
				...
				(.5+5.*m1.x*noise(7.*pos1+vec3(TIME)))*max(0., m2.y-1.)*(1.-cb(12));
		}
		scol *= QNq;
		...
		fragColor = mix(scol.xyzz, texture(BuffA, uv), pow(.0001, 0.016667));
	}
	return fragColor; 
 } 
