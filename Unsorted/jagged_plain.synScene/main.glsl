...
float map(vec3 p){
     ...
     return p.y - surfFunc(p)*1.5 + 1.5*(sin(morph_time*0.25)+1.25) + sin(p.z*waves);
...
vec4 renderMainImage() {
	...
	// Camera Setup.
	vec3 lookAt = vec3(0.0, 2.25, smoothTime);  // "Look At" position.
  ...
  camPos.xy += look*-1;
	...
	lookAt.xy += cam_path*path(lookAt.z);
	camPos.xy += cam_path*path(camPos.z);
	lp.xy += cam_path*path(lp.z);
	lp2.xy += cam_path*path(lp2.z);
	...
	if(t<40.0001){
        ...
        // spoil the illusion.
      vec3 userCol = vec3(0.2, 0.8, 0.3);
      vec3 snUser = userCol;
      vec3 snImg = doBumpMap(image3, sp*tSize0, sn, 0.04);
      sn = mix(snUser, snImg, image_mix);

        // Obtaining the texel color.
      vec3 texUser = userCol;
	    vec3 texImg = tex3D(image3, sp*tSize0, sn);
      vec3 texCol = mix(texUser, texImg, image_mix);
	    ...
	    float atten = 1./(1. + lDist*lDist*0.0175);
	    float atten2 = 1./(1. + lDist2*lDist2*0.0175);
