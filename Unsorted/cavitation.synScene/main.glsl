...
float map(vec3 p) {
    float fft = clamp(( vec2(syn_BassLevel, syn_MidLevel ), vec2(0.1,0.1) ).x * 12., 0.2, 999999.);
    float k;
    vec4 p4 = inverseStereographic(p,k);
    float c = sin(smoothTime*0.01);
    float t = mod(smoothTime *0.05, 333.);
...
vec4 renderMainImage() {
    ...
    const float ITER = 130.;
    const float FUDGE_FACTORR = .4;
