...
vec2 map(vec3 p){
    ...
    vec3 q = p;
    float wave = pow(abs(sin(q.z*0.2+script_time*1.5)), 200.);
    ...
    float mt = sin(smooth_hightime*1. + id.x + sin(id.y)*0.4 + id.y*0.4);
    ...
    for(float i = 0.; i< 5.; i++){
        ...
        if(i == 3.){
            ...
            dp = max(dp, 0.);
            vec3 ccc = pal(1.,1.,vec3(0.8,2.2,2.5),0.5,0.5 + script_time + q.z);
...
vec4 renderPassA() {
    ...
    ro.xy += vec2(cos(TIME*0.6), sin(TIME*0.5)*0.1)*0.4;
