...
float sdVerticalCapsule( vec3 p, float h, float r )
{
    ...
    p.x += clamp(sign(cos(smoothTimeC+p.y*10.))/5.,0.,0.1)*.7  ;
...
{
    p += vec3(sin(smoothTimeC),cos(smoothTimeC),cos(smoothTimeC))/10.; // Sphere will move
...
///// Pre effects that will affect all the objects
     ...
     p += vec3(p.z*sin(-TIME)/12.,cos(TIME)/12., sin(p.y+TIME)+smoothTime);
     //         ^ Yaw
     //             ^ Translation X
     //                               ^ Translation Y
     //                      ^-------------^ 
...
float diffuse_directional(vec3 n,vec3 l,float d){
    ...
    return (a+b)/2.+0.1*abs(tan(d+smooth_hightime));  ; // Why not both ?
...
vec4 renderMainImage() {
    ...
    //uv.y += sign(sin(uv.x*50.))/100.* sin(uv.x*uv.x +uv.y*uv.y) ;//*sin(TIME*2.);
    //uv.x += sign(cos(uv.y*50.))/100. * sin(uv.x*uv.x +uv.y*uv.y);//*cos(TIME*2.);
