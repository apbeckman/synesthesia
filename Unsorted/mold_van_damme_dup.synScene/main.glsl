...
{
    vec3 dx = vec3(-1,0.,1);
    return texel(ch, p+dx.xy)+texel(ch, p+dx.yx)+texel(ch, p+dx.zy)+texel(ch, p+dx.yz)-(4.+MediaFactor)*texel(ch, p);
}
...
vec4 renderPassB() {
    ...
    float video = length(Laplace(syn_UserImage, p*ss)*(1.+0.75*growthFactor));
