...
float getRndID(vec2 p){
    ...
    float rnd = hash21(p);
    return smoothstep(.5, .875, sin(rnd*6.283 + smoothTimeB*0.025)); 
    #else
    return hash21(p) - .75;
    #endif
...
vec3 getNormal(vec3 p, inout float edge, inout float crv, float ef) { 
    ...
    vec2 e = vec2(ef/mix(450., RENDERSIZE.y, .5), 0);
    ...
    e = vec2(.0025, 0); //RENDERSIZE.y - Depending how you want different resolutions to look.
	d1 = map(p + e.xyy), d2 = map(p - e.xyy);
	d3 = map(p + e.yxy), d4 = map(p - e.yxy);
	d5 = map(p + e.yyx), d6 = map(p - e.yyx);
...
vec3 getObjectColor(vec3 p, vec3 n){
    ...
    vec3 col = tex3D(image3, txP*sz0, n );
...
vec3 doColor(in vec3 sp, in vec3 rd, in vec3 sn, in vec3 lp, in float t){
    ...
    if(t<FAR){
        ...
        //txP.xy -= path(txP.z);
        //txP.xz *= r2(getRndID(svVRnd)*6.2831);
        sn = texBump(image3, txP*sz0, sn, .005);///(1. + t/FAR)
...
vec4 renderMainImage() {
	...
	vec3 lk = vec3(0, 1.25, smoothTime*.5);  // "Look At" position.
