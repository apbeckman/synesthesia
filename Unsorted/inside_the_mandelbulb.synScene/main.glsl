...
// --
#define LOOPS   2.5   // 4+ and higher to show off you expensive GPU
#define POWER   6.0
#define ANIMATE
// --
...
float mandelBulb(vec3 p) {
  ...
  for(int i = 0; i < LOOPS; ++i) {
    r = length(z);
    if(r > 2.0) continue;
    theta = atan(z.y+sin(smoothTimeC/10)/10, z.x+cos(smoothTimeC/10)/10);
#ifdef ANIMATE
    phi = asin(z.z / r) + smoothTimeC*0.12;
#else
    phi = asin(z.z / r);
#endif
...
vec3 effect(vec2 p) {
  g_rot = rot_x(0.126*smoothTime)*rot_y(0.123*smoothTime);
  vec3 ro = (0.6*Zoom)*vec3(0.0, 2.0, 5.0);
