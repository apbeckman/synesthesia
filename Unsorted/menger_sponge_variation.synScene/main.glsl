...
vec4 renderMainImage() {
    ...
    if(t<FAR){
		...
		const float ts = 0.5; // Texture scale.
        vec3 oCol = tex3D(image6, sp*ts, sn); // Texture color at the surface point.
        
 		
        // Darker toned wood paneling. Very fancy. :)
        vec3 q = abs(mod(sp, 3.) - 1.5);
        if (max(max(q.x, q.y), q.z) < 1.063) oCol = oCol*vec3(1, 1, 1); 
        ...
        rCol = min(rCol, 1.)*(rDiff + vec3(.5, .56, .57))*rAtten; // Reflected color. Not accurate, but close enough.
        //
    	// END REFLECTION BLOCK.
        

        // Combining the elements above to light and color the scene.
        col = oCol*(diff + vec3(.5, .56, .57)) + vec3(.5, .5, .58)*spec*2. + rCol*0.25;
