...
function update(dt) {
  ...
  bassT = bassT + (inputs.syn_BassLevel*0.35)+(inputs.syn_MidLevel*0.3)+(inputs.syn_Level*0.2)+(inputs.syn_Presence*0.1);
  uniforms.script_bass_time = bassT;
  highT = highT + (inputs.syn_MidHighLevel*0.35)+(inputs.syn_HighLevel*0.2)+(inputs.syn_Level*0.2);
  uniforms.script_high_time = highT;
  uniforms.smooth_basstime = bpmcount.time*0.8+(bassT*0.15);
  uniforms.smooth_hightime = bpmcount.time*0.8+(highT*0.15);
