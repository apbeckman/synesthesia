...
float drawSphere(in vec3 p){
    ...
    p = fract(p)-Smoother; // Default was 0.5 MS: Careful with this, produces weird blue shapes on many values
    return dot(p, p);
...
float map(vec3 p){
    ...
    if (Standard_Tunnel < 0.5) {
     float sf = cellTile(p*Cell_Tile); // Cellular layer. MS: This slider is a lil glitchy

     p.xy -= path(p.z); // Move the scene around a sinusoidal path.
     p.xy = rot2(p.z/Rotation)*p.xy; // Twist it about XY with respect to distance. Rotation default was 12.

     float n = dot(sin(p*1. + sin(p.yzx*.5 + syn_Time*Wave_Gain)), vec3(.25)); // Sinusoidal layer.

     return Clamp - abs(p.y) + n + sf; // Warped double planes, "abs(p.y)," plus surface layers.
   } else {
     // Standard tunnel. Comment out the above first.
     // MS: Standard tunnel is pretty meh but thought I'd still include it just cause
     vec2 tun = p.xy - path(p.z);
     return 3. - length(tun) - (0.5-syn_Level) +  dot(sin(p*1. + sin(p.yzx*.5 + syn_Time*.75)), vec3(.333))*.5+.5;
   }
...
float bumpSurf3D( in vec3 p){
    ...
    float noi = noise3D(p*64.); // MS: Default 64.
    float vor = cellTile2(p*Voronoi_Level); // MS: Default .75
...
float trig3(in vec3 p){
    p = cos(p*2. + (cos(p.yzx) + 1. + TIME*2. + syn_Time*Mist_Gain)*1.57); // Mist
...
float trigNoise3D(in vec3 p){
    ...
    float t = trig3(p*PI);
	  p += (t - syn_Time*0.25); // MS: Not sure what this does, maybe noise?
    p = m3RotTheta*p;
    //p = (p+0.7071)*1.5;
    res += t;

    t = trig3(p*PI);
	p += (t - syn_Time*0.25)*0.7071;
    p = m3RotTheta*p;
     //p = (p+0.7071)*1.5;
    res += t*0.7071;
...
vec4 renderMainImage() {
	...
	vec3 lookAt = vec3(0., 0.0, syn_Time*2. + 0.1);  // "Look At" position.
	...
	if(t<FAR){
        ...
        float per = 10.;
    	float tanHi = abs(mod(per*.5 + t + TIME, per) - per*.5);
