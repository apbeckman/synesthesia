...
float sRGB(float t) { return mix(1.055*pow(t, 1./2.4) - 0.1055, 2.92*t, step(t, 0.0021308))*(pow(1.0+highhits*0.5*flash, 2.)); }
...
vec4 renderMainImage() {
  ...
  vec2 p_rotate = _rotate(_uvc.xy*sin(p.xy)*4, smoothTimeC*0.25);
  ...
  p.xy = mix(p.xy, p_rotate, Warp);
  p.xy = mix(p, p+_uvc.xy*PI, FOV);

  float x_m =  smin(p.x, p.x*-1, 0.5);
  float y_m =  smin(p.y, p.y*-1, 0.5);
  vec2 p_mirrored = vec2(x_m, y_m);
  p_mirrored = mix(p_mirrored, p_mirrored*-1, invert);
  vec2 mirror = vec2(mirror_x, mirror_y); 
  p.xy = mix(p.xy, p_mirrored, mirror);
