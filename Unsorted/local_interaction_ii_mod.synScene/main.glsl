...
vec4 image = texture(syn_UserImage,(_xy)/RENDERSIZE.xy)*.5;
...
vec4 renderPassB() {
    ...
    Q  =  A(U);
    // Neighborhood :
    vec4 pX  =  A(U + vec2(1,0))+growthFactor*image;
    vec4 pY  =  A(U + vec2(0,1))+growthFactor*image;
    vec4 nX  =  A(U - vec2(1,0))+growthFactor*image;
