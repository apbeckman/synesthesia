...
float map(vec3 pos)
{
    ...
    float j = smooth_basstime * speed + (CLUBBER_A);
    float tm = fract(j);
    rmod *= (cos(tm*PI)-1.0);
...
vec4 renderMainImage() {
    ...
    eye = eye * yrot(TIME*0.25+(script_time*0.0125)) * xrot(TIME*0.25+(script_time*0.0125));
    
    float speed = 0.5;
    
    float j = smooth_hightime * speed + CLUBBER_A;
