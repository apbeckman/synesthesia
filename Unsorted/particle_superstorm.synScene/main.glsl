...
vec4 renderPassA() {
   ...
   // if(length(U.xy - pos) > 7.)
    ...
    vec2 vel0 = pspeed*vec2(cos(U.z), sin(U.z)) + 0.1*(hash22(U.xy+TIME*0.1)-0.5);
    vec4 F =  pixel(ch1, U.xy);
    vec2 vel1 = F.yz;
    float dangl = 0.1*F.x*angle_between(vel0,vel1);
    U.z += dt*(dangl+0.1*(hash12(U.xy+TIME*0.1)-0.5)+0.4*sin(U.w));
   // U.w += dt*0.4;
   
    vec2 pvel = pspeed*vec2(cos(U.z), sin(U.z)) + 1.*(hash22(U.xy+TIME*0.1)-0.5);;
    ...
    if(FRAMECOUNT <= 1 || (_mouse.z > 0. && length(_mouse.xy - pos) < 30.) || (length(size.xy*0.5 - pos) < 30.))
    {
