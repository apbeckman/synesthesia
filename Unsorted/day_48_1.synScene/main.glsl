...
#define mx (TIME*0.9 + 20.*_mouse.x/RENDERSIZE.x)
...
vec2 map(vec3 p){
    ...
    #define modDist 1.+Dist

    #define tunnW 0.7+TunnelWidth

    #define pipeW 0.04+PipeWidth
    ...
    p.xy *= rot(0.4 + p.z*(0.1 + sin(smoothTimeC*0.05) )+ smoothTimeC*0.05);
        ...
        if (d.y < 8.*(1.0+highhits*0.25)){
            ...
            glow += mix(
                ...
                mix(
                    ...
                    exp(-d.x*0.01)*pal(2.8,1.4,vec3(5.8,2.4+modu*0.2,0.8+ sin(p.y + smoothTimeB)*.09), 6.9 +modu*0.2 + sin(p.z)*.5,0.5 + t*0.5)*2.,
        	...
        	glow += exp(-d.x*2.)*pal(0.2,1.9,vec3(1.8,0.4,0.8), 5.6 ,2.- t*0.1)*0.4*(1.0+highhits*0.25);
