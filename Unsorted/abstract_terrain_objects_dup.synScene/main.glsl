...
#define FAR pow(Fog,2) // MS: Default 100
...
vec2 path(in float t){
    ...
    //return vec2(s*4., 0.);
//plane time
    float a = sin(t*Sin_Warp+(smoothTimeC*0.125)); //.12
    float b = cos(t*Cos_Warp+(smoothTimeC*0.125)); //.14
...
// MS: This isn't what you think. This is triangle noise and fucking with it totally deconstructs the scene
...
// Height function layers with some optional audioreactivity
float h1(vec2 p){ return sin(1.57079632679+smoothTimeC*0.125)*0.5*dot(tri(p + tri(p.yx*.5 + .25)), vec2(1)); }
float h1Low(vec2 p){ return sin(1.57079632679-smoothTimeC*0.125)*0.5*dot(triS(p + triS(p.yx*.5 + .25)), vec2(1)); }
...
float sdPyramid( vec2 p, float h)
{
  float m2 = h*h + 0.25;
  ...
  vec2 q = vec2(h*p.y - 0.5*p.x, h*p.x + 0.5*p.y);

  float s = max(-q.x,0.0);
  float t = clamp( (q.y-0.5*p.x)/(m2+0.25), 0.0, 1.0 );
  ...
  return sqrt( (d2+q.x*q.x)/m2 ) * sign(max(q.x,-p.y));
}
...
// MS: Trying to implement Theron's mix method with some weird results
    float diamond = (p.x + p.y)*.7071;
    float square = max(p.x, p.y);
    float hexagon = max(p.x*.866025 + p.y*.5, p.y);
    float octagon = max((p.x + p.y)*.7071 - .4, max(p.x, p.y));
    float weirdShape = max(p.x + p.y, p.x*-.4 + p.y/.3);
    vec3 triInput = vec3(p.x,p.y,p.x);
    float oval = udTriangle(triInput, triInput/2, triInput/2, triInput/2);

    float triangle = max(abs(p.y),max(abs(p.x)*0.866025+abs(p.y),abs(p.y))-abs(p.x)*0.5);

    float finalShape = diamond;

    if (Shaper <= 1) finalShape = mix(diamond, hexagon, clamp(Shaper,0,1));
    else if (Shaper <= 2) finalShape = mix(hexagon, octagon, clamp(Shaper,1,2)-1);
    else if (Shaper <= 3) finalShape = mix(octagon, weirdShape, clamp(Shaper,2,3)-2);
    else if (Shaper <= 4) finalShape = mix(weirdShape, triangle, clamp(Shaper,3,4)-3);
    else finalShape = mix(triangle, square, clamp(Shaper,4,5)-4);

    return finalShape * Mirror_Size;

    //return max(p.x + p.y, p.x*(Hex+Hex-1) + p.y/Hex)*Mirror_Shape; // Diamond = 0.7071, Square = 1
...
vec3 sc = vec3(16, 4, 4+Object_Distance);
...
float map(vec3 p){
    ...
    float terr = p.y + .0 + sf + Object_Height;
...
float trace(vec3 ro, vec3 rd){
    ...
    int numSteps = int(pow(Ray_Steps,3) * 80);
...
vec3 getObjectColor(vec3 p, vec3 n){
    ...
    if(svObjID==1. || abs(bordTx0Tx1)<bordW) tx1 = vec3(.08, .1, .12) + hueShift(vec3(Colorize_Objects*.5, 0, Colorize_Objects),syn_Time*Hue_Time); // MS: Adding ability to get a purple hue
...
vec3 doColor(in vec3 ro, in vec3 sp, in vec3 rd, in vec3 sn, in vec3 lp, float edge, float crv, float ao, float t){
    ...
    //    / spec += highhits*15.*Light_Hits;
        ...
        if(svObjID==1.) diff *= diff*(2.+highhits*2.);
        if(svObjID==1.) spec *= spec + highhits*30.;
...
vec4 renderMainImage() {
	...
	// Camera Setup.
	vec3 ro = vec3(0, 1.25, smoothTime); // Camera position, doubling as the ray origin.
	vec3 lk = ro + vec3(0., 0.,.5);  // "Look At" position.


    // Light position. Set in up in the sky above the horizon -- out near the far plane.
    vec3 lp = vec3(0);
    if (Light_Hits > 0.5) { lp = ro + vec3(-20, 30, 60*basshits*(.25+Light_Gain)); }
    else { lp = ro + vec3(-20, 30, 60); }
    ...
    float FOV = 3.14159/FOV; // FOV - Field of view.
    ...
    // Toggle check for refraction
    if (Refract > 0.5) { refl = refr; }
    ...
    */
    ...
    float overlay = 1. + .35*sin(u2.x*3.14159*RENDERSIZE.y/1.5);
    overlay *= 1. + .35*sin(u2.y*3.14159*RENDERSIZE.y/1.5);
