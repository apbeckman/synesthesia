...
*/
#define FAR 40. // Far plane, or maximum distance.
...
float map(vec3 p) {
    ...
    p = cos(p * .315 * 1.25 + sin(p.zxy * .875 * 1.25) + sin(smoothTimeC * 0.05) * 0.2); // 3D sinusoidal mutation.
    float n = length(p + _noise(_uv+TIME*0.3)*0.35); // Spherize. The result is some mutated, spherical blob-like shapes.
...
float trace(in vec3 ro, in vec3 rd) {
    ...
    for(int i = 0; i < 96 - liquify * liquify * 60; i++) {
        ...
        if(abs(h) < 0.001 * (t * .125 + 1.) || t > FAR)
            break; // Alternative: 0.001*max(t*.25, 1.)
        t += h;

        // Simple distance-based accumulation to produce some glow.
        if(abs(h) < .35)
            accum += (.35 - abs(h)) / 58.;
            accum *= 1.0 + glow*0.05+Flash*0.05*pow(0.95 * syn_HighLevel + 0.75 * syn_MidHighLevel + syn_HighHits * 0.24, 2.);
...
vec4 renderMainImage() {
   ...
   // lk.xy += n2D(vec2(cos(TIME*0.125), sin(TIME*0.2))*0.5)*0.25;
    ...
    lk.xy = mix(lk.xy, lk.xy + _uvc, FOV );
    ...
    float inv_geo = mix(1.0, -1.0, invert_geometry);
    // Unit direction ray.
    //vec3 r = normalize(fwd + FOV*(u.x*rgt + u.y*up));
    // Lens distortion.
    vec3 r = (0.1 + FOV) * (fwd + (u.x * rgt - FOV * _uvc.x - FOV * _uvc.y + u.y * up));
    float screen_mirror_x = smin(r.x * -1, r.x, 0.125);
    float screen_mirror_y = smin(r.y * -1, r.y, 0.125) ;
    vec2 smxy = vec2(screen_mirror_x, screen_mirror_y)*inv_screen;
    vec2 sm = vec2(x_mirror_screen, y_mirror_screen);
    
    r.xy = vec2(mix(r.x, smxy.x, sm.x), mix(r.y, smxy.y, sm.y));

    r.xy = _rotate(r.xy, Rotation*PI);
    // r.xy += vec2(_noise(_uv+TIME*0.1))*0.2;

    // r = normalize(vec3(n3D(r.xyz) * 0.1 + r.xy * (1.0) + warp * vec2(_noise(_uv + TIME * 0.1)) * 0.1, (r.z - length(r.xy - _uvc * PI * Fisheye + warp * vec2(_noise(_uv + TIME * 0.1)) * 0.1) * .125)));
    r.z -= mix(0, length(- _uvc*0.5), Fisheye);
    r = normalize(vec3(r.xy, (r.z - length(r.xy)*.125)));
    r.yz = _rotate(r.yz, lookXY.y * PI);
    r.xz = _rotate(r.xz, 1.0 * lookXY.x * PI);
    float mirror_x = smin(r.x * -1, r.x, 0.25);
    float mirror_y = smin(r.y * -1, r.y, 0.25) ;
    vec2 m = vec2(x_mirror, y_mirror);
    vec2 mxy = vec2(mirror_x, mirror_y)*inv_geo;

    for(int i = 0; i < 3; i++){
        r.xy = vec2(mix(r.x, mxy.x, m.x), mix(r.y, mxy.y, m.y));
    }
