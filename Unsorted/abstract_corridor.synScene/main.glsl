...
#define FH 1.0+Floor_Height // Floor height. Set it to 2.0 to get rid of the floor.
...
vec4 renderMainImage() {
	...
	// Camera Setup.
	vec3 camPos = vec3(0.0, 0.0, smoothTime*1.); // Camera position, doubling as the ray origin.
	...
	if(dt<0.005){
	    ...
	    float distlpsp = max(length(ld), 0.0001);
	    float distlpsp2 = max(length(ld2), 0.0001);
