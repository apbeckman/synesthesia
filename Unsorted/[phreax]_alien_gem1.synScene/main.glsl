...
vec3 transform(vec3 p) {
    ...
    p *= 1.5;
    p.xy=-_rotate(p.xy,mid_time); 

    p.xy = foldSym(p.xy, 6+Symmetry);
...
vec3 transform2(vec3 p) {
    ...
    p *= .8;
    p.xy=_rotate(p.xy,rot_time); 

    p.xy = foldSym(p.xy, 3.+Symmetry);
    p.xy = abs(p.xy)-0.3*SIN(-.3*tt);  
    p.yz *= rot(PI*.25);
    p.xy = _rotate(p.xy, fold_time);

    float r = 1.+Radius;
    vec2 cp = vec2(length(p.xz)-r, p.y);
 

    float rev = 1.+revolutions;
    float a = atan(p.z, p.x);
