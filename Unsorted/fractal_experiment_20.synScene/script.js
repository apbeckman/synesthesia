...
function update(dt) {
  ...
  try {
    ...
    midTimevar.updateTime(0.2, Math.pow((inputs.syn_BassLevel*0.785+inputs.syn_MidHighLevel*0.9+inputs.syn_MidLevel*1.5+inputs.syn_Intensity*0.5), 2.0)*(inputs.mid_time), dt);
    spinTimevar.updateTime(.4,  Math.pow(0.5+inputs.syn_BassLevel*0.75+inputs.syn_MidLevel*0.75+syn_Intensity,2.0)*(inputs.Rotate), dt);

    timevar.updateTime(0.4, inputs.rate_in, dt);

    uniforms.script_time = timevar.time;
    uniforms.bass_time = bassTimevar.time;
    uniforms.midtime = midTimevar.time;
