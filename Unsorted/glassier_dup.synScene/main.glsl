float bassHits = normalize(basshits);
float highHits = normalize(highhits);
float vuTimeUncorrected = syn_Time;
float bassTimeUncorrected = smoothTime;
...
float iGlobalTime = smoothTime*2.0;
...
void mainImage0(out vec4 fragColor, in vec2 fragCoord)
{
 ...
 /* if (auto_position >= 0.5){
  ...
  float twitcher = (highhits*auto_impulse+manual_impulse*0.7)*impulse_power;
  ...
  vec2 d = pixelSize*16.;
  vec4 dx = (texture(buffC, fract(uv + vec2(1,0)*d)) - texture(buffC, fract(uv - vec2(1,0)*d))) * 0.25;
  vec4 dy = (texture(buffC, fract(uv + vec2(0,1)*d)) - texture(buffC, fract(uv - vec2(0,1)*d))) * 0.25;
  // if (flowing == 1.0){
