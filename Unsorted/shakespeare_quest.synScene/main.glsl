...
{//the segment lengths are precalculated as 0.5,0.4,0.3 and starts at point 0,0
 ...
 p=p-j1;
 q=p*(0.5+0.0135/dot(p,p));
 vec2 j2=j1+q+sqrt(max(0.0,0.16/dot(q,q)-1.0))*q.yx*vec2(-1.0,1.0);
 return vec4(j1,j2);
}
mat3 lookat(vec3 fw,vec3 up){
 fw=normalize(fw);vec3 rt=normalize(cross(fw,up));
...
vec3 getColor(vec3 p0){
 ...
 if(d2<d && d2<d3){col=vec3(0.72,0.72,0.2);} //bar color
 else if(d3<d){
  p0.z+=0.125;
  vec2 xz=abs(mod(p0.xz,0.25)-0.125);
  col=(1.0-20.0*dot(xz,xz))*vec3(0.5,0.4,0.3); //floor base color
  xz=floor(p0.xz*4.0);
...
vec3 Light(vec3 p, vec3 rd, vec3 L){
 ...
 col+=vec3(1.0,0.75,0.7)*pow(max(0.0,dot(reflect(rd,N),L)),32.0);
#ifdef SHADOWS
...
vec4 renderMainImage() {
 ...
 time=smoothTimeC*.25;
 float Btime=TIME*12.5;
 float time=bass_time*2.5;
#ifdef SHADOWS
 ...
 vec3 ro=vec3(2.0,1.5+Orbit.y,time*0.6+cos(time*0.135));
 mat3 rotCam=lookat(vec3(0.0+Orbit.x,0.3-0.5*ro.y,1.0),vec3(0.0,1.0,0.0)-vec3(_uvc.x*FOV, _uvc.y*FOV, 0));

 vec3 rd=rotCam*normalize(vec3((2.0*fragCoord.xy-size.xy)/size.y,1.5+Zoom));
 rd.x=-rd.x;

 float t=max(0.0,(0.9875-ro.y)/rd.y),d=1.0,dm=d,tm=t;
 for(int i=0;i<24;i++){
  t+=d=DE(ro+rd*t);
  if(d<dm){dm=d;tm=t;}
 }
 ...
 if(secondMarch){//unless we got really close speed up the march
  ...
  if(d<0.01*t){
   ...
   col=mix(scol,col,smoothstep(0.0,0.01*t,d));
   col*=exp(-t*0.1);
  }
 }
 if(dm<0.001*tm){
  vec3 p=ro+rd*tm;
  vec3 scol=Light(p,rd,L);
  col=mix(scol,col,smoothstep(0.0,0.001*tm,dm));
  col*=exp(-tm*0.1);
 }
