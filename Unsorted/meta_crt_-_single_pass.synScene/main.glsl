...
{   
    ...
    vec3 vPixelEmissive = textureLod( syn_UserImage, vSampleUV.xy, 0.0 ).rgb;
...
vec4 Env_GetSkyColor( const vec3 vViewPos, const vec3 vViewDir )
{
    ...
    vResult.rgb = mix( vec3(0.0,0.0,0.0),vec3(0.3,0.4,0.5), vViewDir.y );
...
vec4 renderMainImage() {
	vec4 vFragColor = vec4(0.0);
	vec2 vFragCoord = _xy;

    vec2 vUV = vFragCoord.xy / RENDERSIZE.xy; 
...
//#if USE_MOUSE    
    ...
    {
        float fDist = 0.01 + 3.0 *Zoom;
        float fAngle = (Rotate.x) * radians(360.0);
    	//float fElevation = (_mouse.y / RENDERSIZE.y) * radians(90.0);
    	float fElevation = 0.15f * radians(90.0)*Rotate.y*2.;    
...
*/    
    ...
    vec2 vJitterUV = vUV + cam.vJitter / RENDERSIZE.xy;
    Cam_GetCameraRay( vJitterUV, RENDERSIZE.xy, cam, vRayOrigin, vRayDir );
