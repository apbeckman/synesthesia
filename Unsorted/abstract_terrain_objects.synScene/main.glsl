...
vec2 path(in float t){
    ...
    float a = sin(t*Sin_Warp+(warp_time)); //.12
    float b = cos(t*Cos_Warp+(warp_time)); //.14
...
float h1(vec2 p){ return sin(1.57079632679+plane_time)*dot(tri(p + tri(p.yx*.5 + .25)), vec2(1)); }
float h1Low(vec2 p){ return sin(1.57079632679-plane_time)*dot(triS(p + triS(p.yx*.5 + .25)), vec2(1)); }
...
vec4 renderMainImage() {
	...
	// Camera Setup.
	vec3 ro = vec3(0, 1.25, fly_time); // Camera position, doubling as the ray origin.
	vec3 lk = ro + vec3(LookXY.xy, .5);  // "Look At" position.


    // Light position. Set in up in the sky above the horizon -- out near the far plane.
    vec3 lp = vec3(0);
    if (Light_Hits > 0.5) { lp = ro + vec3(-20, 30, 60*syn_BassHits*(.25+Light_Gain)); }
