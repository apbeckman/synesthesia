...
#define T(p) texelFetch(BuffA, ivec2(mod(p,R)), 0)
#define P(p) texture(BuffB, mod(p,R)/R)
...
#define oscil_pow 1.
float sense_force = 0.11 *(1.0 + basshits*1.5);
...
{
	return texelFetch( isk, ivec2(KEY,0), 0 ).x > 0.5;
}
*/
vec4 renderPassA() {
	vec4 U = vec4(0.0);
...
bool isKeyPressed(int KEY)
{
	return texelFetch( int(mouse.xy), ivec2(KEY,0), 0 ).x > 0.5;
}
*/
vec4 renderPassD() {
	vec4 fragColor = vec4(0.0);
...
vec4 renderMainImage() {
	...
	vec2 pos = _xy;
        ...
        vec2 uv = _mouse.xy/R;
        vec2 angles = vec2(-1.5 + 0.03*smoothTime, -0.4)*PI;
        ...
        //tracking particle
        vec4 fp = vec4(R*0.5 + 0.*vec2(150.*smoothTime, 0.), 0., 0.);
