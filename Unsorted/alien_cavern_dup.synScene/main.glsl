...
vec2 map(vec3 p)
{
	...
	float t = sin(smoothTimeC*0.125)*.5+.5;
	t*=2.;

	float rugo = cos(2.*p.x+smoothTimeC*0.5)*sin(1.5*p.z)*sin(3.*p.y+(syn_BeatTime*0.5+smoothTimeC*0.5)*beat_twitching)*cos(1.3);
