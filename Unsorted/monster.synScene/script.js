...
function update(dt) {
  ...
  try {
    bassTimevar.updateTime(0.125, Math.pow((Math.max(0.5, inputs.syn_BassLevel)+inputs.syn_MidLevel*0.75+syn_Intensity*0.25), 2.0)*(inputs.rate_in+inputs.syn_Intensity), dt);

    midTimevar.updateTime(0.125, Math.pow((Math.max(0.5, inputs.syn_BassLevel)+inputs.syn_MidHighLevel*0.9+inputs.syn_MidLevel*1.5+syn_Intensity), 2.0)*(inputs.mid_time), dt);
