...
vec2 map(vec3 p){
    ...
    float attA = pow(abs(sin(sin(p.z + (script_time*0.6)) + p.z*0.7 + sin(p.y*1.5) + smooth_basstime*0.24) ), 10.);
    float attB = pow(abs(sin(TIME*0.2 + sin(p.z + (script_time*0.6)) + p.z*0.4 + sin(p.y*1.5))), 10.);
...
vec4 renderPassA() {
    ...
    vec3 ro = vec3(0);
    ro.z += (smoothTime)*SPEED*0.45+(TIME*0.125);
    ro.z += mx;
        
    ro += path(ro.z);
    
    float wave = pow(abs(sin(ro.z*0.2 + smooth_hightime*0.75 + 0.17)), 20.);
    ...
    if(hit){
        ...
        if(d.y >= 10.){
            ...
            c = max(c, 0.);
            float a = pmod(p.z + (smoothTimeC*0.00125), 1.)/1.;
