...
vec4 Truchet(vec2 p){
    ...
    vec4 h2 = getHex(p - vec2(0, .9660254*2./13.));
    #endif
...
vec4 renderMainImage() {
	...
	// Camera Setup.
	vec3 lk = vec3(0, 0, ((bass_time*0.5)));  // "Look At" position.
    lk.xy += lk.xy*_uvc;
   ...
   // o.xy +=Warp*_uvc/PI;
    ...
    // r - Ray direction.
    vec3 r = normalize(forward + u.x*right + u.y*up);
    r = normalize(vec3(n3D(r.xyz)*0.1+r.xy, (r.z - length(r.xy+_uvc*PI*Fisheye)*0.5)));
    ...
    if(t<FAR){
       ...
       // col = tx; //vec3(1)*fBm; // Initializing to the texture color.
        ...
        if(svID>.5){
            vec3 truchetColor1 = vec3(.1*_uvc.x, 0.1*_uvc.y, .1);
            //vec3 truchetColor1 = vec3(.77, .44, .24);
            vec3 truchetColor2 = vec3(.87);

           // col *= max(1. - bumpFunc(p, n)*1.5, 0.);
        	//col *= mix(vec3(.55, 0.542, .453)*(1.0+syn_HighLevel), vec3(.01681, 0.0125, .14682), bumpFunc(p, n)*1.5);
        	col *= mix(truchetColor1*(3.0+syn_HighLevel), truchetColor2*0.25, bumpFunc(p, n)*1.5);
            //col *= vec3(1, .1, .2)*1.5;
                 
        }
        else {
            col *= 0.+max(0.5 - bumpFunc(p, n)*1.5, 0.);
            //col *= vec3(.08, .06, .024);
            //col *= vec3(.08, .06, .024);
        }
    ...
    col = mix(col, fogCol, smoothstep(0., .995, t/FAR)); // exp(-.002*t*t), etc.
    
    
    // Vignette.
    u = fCoord/RENDERSIZE.xy;
