...
float highsAccum = syn_HighLevel*0.75+0.75*syn_Intensity;
float bassAccum = syn_BassLevel*0.75+0.75*syn_Intensity;
...
{
  fragCoord += _uvc*PI*Zoom*(1.0+syn_BassLevel*0.2+syn_Intensity*0.2);
  fragCoord += PI*MoveXY*(1.0+syn_BassLevel*0.2+syn_Intensity*0.2);
  fragCoord += PI*Stretch*_uvc*(1.0+syn_BassLevel*0.2+syn_Intensity*0.2);
  ...
  float twitcher = (syn_HighHits*syn_Intensity*auto_impulse+manual_impulse*0.7)*impulse_power;
  ...
  vec4 noise = vec4(_noise(_uv*10000.0),_noise(_uv*1000.0),_noise(_uv*10000.0),_noise(_uv*10000.0));
...
void mainImage4( out vec4 fragColor, in vec2 fragCoord ){
  ...
  vec4 noise = vec4(_noise(_uv*01000.0),_noise(_uv*10000.0),_noise(_uv*10000.0),_noise(_uv*10000.0));
  ...
  vec3 botLightCol = vec3(0.5,0.16,0.7)*2.5;
  ...
  // *** Color Regime 3 ***
  baseCol = mix(baseCol, vec3(0.5,0.5,01.5), cReg3);
