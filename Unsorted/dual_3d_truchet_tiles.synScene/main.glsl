...
vec4 renderMainImage() {
    ...
    if(t<FAR){
        ...
        vec3 oCol = vec3(1);
        vec3 bCol = mix(vec3(1, .1, .3).zyx, vec3(1, .5, .1).zyx, dot(sin(sp*8. - cos(sp.yzx*4. + TIME*4.)), vec3(.166)) + .5);
        ...
        sc += .015/max(abs(.05 - map(sp*1.5 + sin(TIME/6.))), .01)*oCol*mix(bCol, vec3(1, .8, .5), .35);
