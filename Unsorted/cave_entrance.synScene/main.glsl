...
float map(vec3 p){
     ...
     p.y += z_waves*sin(morph_time + p.z);
     p.y += y_waves*sin(morph_time + p.x);
     //p.x += x_waves*sin(morph_time + p.z);
     p.x -= x_waves*cos(morph_time - p.z);
...
// used for the bump map
vec3 usermedia_or_black(vec3 sp, vec3 sn, float tSize0) {
  vec3 color = vec3(.2, .2, .4);
  if (_exists(syn_UserImage)){
    color = doBumpMap(syn_UserImage, sp*tSize0, sn, 0.01);
  }
  return color;
}

// used for the texture
vec3 usermedia_texture(vec3 sp, vec3 sn, float tSize0) {
  vec3 color = vec3(1.2, 0.4, 1.6);
  if (_exists(syn_UserImage)){
    color = tex3D(syn_UserImage, sp*tSize0, sn);
  }
  return color;
}
...
vec4 renderMainImage() {
	...
	// Camera Setup.
	vec3 lookAt = vec3(1., 0.0, fly_time + 1.);  // "Look At" position.
	vec3 camPos = lookAt + vec3(.0, 0.1, -1.0); // Camera position, doubling as the ray origin.
  camPos.xy += vec2(look.x*-3,look.x*-1);
    ...
    // I guess. All of it is fake, of course, so feel free to swap in a directional light setup.
 	vec3 lp = camPos + vec3(16.0, 4*sin(light_time), 8. + 4*sin(light_time));// Put it a bit in front of the camera.

    // Using the above to produce the unit ray-direction vector.
    float FOV = PI/fov; // FOV - Field of view.
	...
	//rd.xy *= rot2( smoothstep(0.3, 0.7, sin(TIME*0.1875 - 2.)*0.5+0.5)*3.14159 + sin(TIME*0.1875 -2.)*0.1 );
  ...
  int numSteps = int(56 - 52*pow(ray_march,.5));
	...
	if(t<FAR){
      ...
      vec3 snUser = usermedia_or_black(sp, sn, tSize0);
	    sn = mix(snUser, doBumpMap(image2, sp*tSize0, sn, 0.01), texture_mix);
      ...
      vec3 userTex = usermedia_texture(sp, sn, tSize0);
	    vec3 texCol = mix(userTex, tex3D(image2, sp*tSize0, sn), texture_mix);//*0.66+tex3D(texChannel0, sp.yzx*tSize0*2., sn)*0.34;
    ...
    float fogAtten = 1./(1.+t*t*0.025*fog);
    sceneCol *= fogAtten;
	...
	float hLayers = 8. + 2*sin(light_time*.25);
	heat = floor(heat*(hLayers-0.001))/(hLayers-1.);
	vec3 heatCol = vec3(heat);//vec3(min(heat*1.5, 1.), pow(heat, 2.5), pow(heat, 10.));

  sceneCol = mix(sceneCol, heatCol, heat_map);
