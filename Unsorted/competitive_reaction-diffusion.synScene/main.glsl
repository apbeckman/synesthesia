...
image *= MediaImpact;
    ...
    if (_mouse.z > 0.0) {
    	float mLen = length(_mouse.xy - fragCoord.xy);
    	a += exp(-mLen * mLen / 1000.0);
    }
    
    // initialize
    if(FRAMECOUNT<=10) {
        float mLen = length(RENDERSIZE.xy / 2.0 - fragCoord.xy);
...
vec4 renderPassB() {
    ...
    float _K0 = -20.0/6.0; // center weight
    float _K1 = 4.0/6.0; // edge-neighbors
    float _K2 = 1.0/6.0; // vertex-neighbors
    float timestep = 0.01;
    float death = 0.1+Death;
    float birth = 3.5+Birth;
    ...
    vec4 uv =    texture(BuffB, vUv);
    vec4 uv_n =  texture(BuffB, vUv+n);
    vec4 uv_e =  texture(BuffB, vUv+e);
    vec4 uv_s =  texture(BuffB, vUv+s);
    vec4 uv_w =  texture(BuffB, vUv+w);
    vec4 uv_nw = texture(BuffB, vUv+nw);
    vec4 uv_sw = texture(BuffB, vUv+sw);
    vec4 uv_ne = texture(BuffB, vUv+ne);
    vec4 uv_se = texture(BuffB, vUv+se);
    
    vec4 fn =    texture(BuffA, vUv);
    ...
    if (_mouse.z > 0.0) {
    	float mLen = length(_mouse.xy - fragCoord.xy);
        float rbw = sign(_mouse.x - fragCoord.x);
    ...
    if(FRAMECOUNT<=10) {
    	float mLen = length(RENDERSIZE.xy / 2.0 - fragCoord.xy);
        float rbw = sign(RENDERSIZE.x / 2.0 - fragCoord.x);
...
vec4 renderMainImage() {
    ...
    vec2 rb = texture(BuffB, uv).xy;
    vec3 texCol = vec3(0.2, 0.20, .20);
