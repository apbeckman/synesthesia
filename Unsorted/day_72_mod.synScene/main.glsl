...
vec2 map(vec3 p){
    ...
    p.xy *= rot(sin(p.z*0.9 + sin( p.x*2. + p.z*4. + smoothTime*0.1)*0.9 + p.z*0.1)*0.6*(1.0+sin(smoothTimeC*0.15)*0.125));
    ...
    float atten = pow(abs(sin(z.z*0.82 + smoothTimeB*0.02)), 10.);
    float attenB = pow(abs(sin(z.z*0.92  + sin(z.x + smoothTimeB)*0.02 + sin(z.y*3.)*1. + smoothTimeB*0.05)), 100.);
    float attenC = pow(abs(sin(z.z*0.71  + sin(z.x + smoothTimeB)*0.2 + sin(z.y*3.)*4. + smoothTimeB*0.02)), 200.);

    vec3 col = pal(0.32,0.126 - attenC*0.75,vec3(0.1 + pow(abs(sin(smoothTimeB*.0125)), 40. )*0.005,2.2,0.3),0.5 + sin(smoothTimeB*.0125)*0.005,0.5 - attenB*0.16);
...
vec4 renderPassA() {
    ...
    float m = pow(abs(sin(smoothTimeB*0.0125)), 2.);
    ...
    glowB = max(glowB, 0);
    glowB = pow(glowB, vec3(1./0.45)*.750+pow((syn_HighLevel+syn_MidHighLevel)*(0.25+(syn_Intensity*0.35)), 2.0)/2);
    col += glowB*0.00074;
    
    col += glowC*0.004;
    col += (glow*0.1)*(1.0+highhits);
    col = mix(col, vec3(0.55,0.25,0.2)*0.01, pow(smoothstep(0.,1., firstT*0.028), 2.) );
...
vec4 renderMainImage() {
    ...
    // Radial blur
    float steps = 10.;
    float scale = 0.00 + pow(length(uv - 0.5)*1.2,2.7)*0.12;
    //float chromAb = smoothstep(0.,1.,pow(length(uv - 0.5), 0.3))*1.1;
    float chromAb = pow(length(uv - 0.5),1.4)*(4.5*(0.50+0.5*((syn_BassLevel*2.+syn_Hits*2.)*1.+syn_Intensity)));
    vec2 offs = vec2(0);
    vec4 radial = vec4(0);
