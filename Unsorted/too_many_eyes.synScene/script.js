...
var x_time = new Timer();
var y_time = new Timer();
...
function update(dt) {
  ...
  try {
    ...
    x_time.updateTime(.1,  Math.pow(0.5+inputs.syn_BassLevel*0.75+inputs.syn_MidLevel*0.75+syn_Intensity,2.0)*(inputs.RotateXY.x), dt);
    y_time.updateTime(.1,  Math.pow(0.5+inputs.syn_BassLevel*0.75+inputs.syn_MidLevel*0.75+syn_Intensity,2.0)*(inputs.RotateXY.y), dt);
    spinTimevar.updateTime(.1,  Math.pow(0.5+inputs.syn_BassLevel*0.75+inputs.syn_MidLevel*0.75+syn_Intensity,2.0)*(inputs.Spin), dt);
    ...
    uniforms.x_time = x_time.time;
    uniforms.y_time = y_time.time;
    uniforms.rot_time = spinTimevar.time;
