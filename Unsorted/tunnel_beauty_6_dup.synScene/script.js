...
function update(dt) {
  ...
  bassT = bassT + (inputs.syn_BassLevel*0.4) + (inputs.syn_MidLevel*0.35) + (syn_Level *.25);
  uniforms.script_bass_time = bassT;
  highT = highT + Math.pow( (inputs.syn_MidHighLevel*0.35) + (inputs.syn_HighLevel*0.35) + (syn_Level * .25) ,2.0);
  midT = midT + Math.pow(inputs.syn_MidLevel*0.35+inputs.syn_MidHighLevel*0.15,2.0);

  uniforms.script_high_time = highT;
  uniforms.smooth_basstime = bpmcount.time*0.85+(bassT*0.25);
  uniforms.smooth_hightime = bpmcount.time*0.85+(highT*0.25);
