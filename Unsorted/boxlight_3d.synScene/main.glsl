...
vec2 iChannel1 = vec2(syn_Level, syn_BassLevel);
// Author: ocb
// Title: Boxlight 3D
...
float Mmap(in vec3 tex) { return 6.*tex.r*tex.r; }
...
vec4 trace(in vec3 pos, in vec3 ray, inout vec3 litflux, inout vec3 tex, inout float dAA)
{
    ...
    for(int i = 0;i<MAXSTEP;i++){	// entering the voxel run
        ...
        tex = grid(p.xz,smoothTimeC*0.25);		// get texture at current time.
        ...
        float dhdt = map(grid(p.xz,smoothTimeB*0.6-.02*p.y+.5+.05*H2(floor(p.xz*20.))));
        ...
        litflux += .1*wall.w*wall.w					// to round corner of light flux
            		...
            		*min(1.,20./t)*step(0.,dh);		// cutoff with distance and below the box
...
vec3 getCamPos(in vec3 camTarget){
    float 	rau = 50.,
            alpha = iMouse.x/RENDERSIZE.x*4.*PI,
            theta = iMouse.y/RENDERSIZE.y*PI+(PI/2.0001);	
...
vec4 renderMainImage() {
    ...
    float ti = smoothTime*.125;
    
    // camera def
    vec3 camTarget = vec3(-50.*sin(.4*ti),25.*iMouse.y/RENDERSIZE.y,-30.*cos(.6*ti));    
    ...
    if(dAA<1.){				// if dAA = 1, full object hit, no need for AA
        ...
        float dhdt = map(grid(p.xz,smoothTimeB*0.25 -.3*p.y+.5));
