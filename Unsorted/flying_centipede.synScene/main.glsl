...
vec2 body( vec3 p, const bool complex, out vec4 albedo, float parm, int index ) {
    ...
    if ( complex ) {
        float tex1 = textureLod(image47, inShell.xz*1.0 + float(index)*0.1424, 0.0).r;
        float tex2 = textureLod(image47, inShell.xz*1.4 + float(index)*0.1424, 0.0).r;
...
vec4 getColor( in vec2 fragCoord, in vec3 from, in vec3 dir ) {
    ...
    // get random stuff
    vec3 rnd = hash33(vec3(fragCoord, FRAMECOUNT*2+0));
...
void mainVR( out vec4 fragColor, in vec2 fragCoord, in vec3 fragRayOri, in vec3 fragRayDir ) {
    ...
    // add noise
    vec3 rnd = hash33(vec3(fragCoord, FRAMECOUNT*2+1));
...
vec4 renderMainImage() {
    ...
    vec2 uvv = fragCoord / RENDERSIZE.xy * 2.0 - 1.0;
