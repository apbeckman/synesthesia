...
vec3 d3Noise (vec3 p) {
    vec2 fragCoord = _xy;
    vec2 position = fragCoord / RENDERSIZE.xy;
    ...
    // float value = perlinNoise(vec3(position, TIME * 0.25) * frequency, seed); // single octave perlin noise
    float value = perlinNoise(vec3(position, TIME * 0.25), 1, 6, 0.5, 2.0, seed); // multiple octaves
...
vec3 n3DT(vec3 p){ return d3Noise(p); }
...
float softShadow(vec3 ro, vec3 lp, vec3 n, float k){
    ...
    for (int i = min(ZERO, 0); i<maxIterationsShad; i++){
...
vec4 renderMainImage() {
	...
	if(t < FAR){
        ...
        vec3 refTx = (texCol*1.5 + .66);//smoothstep(.2, .5, refTx);
