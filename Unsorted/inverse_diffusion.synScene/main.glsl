...
vec2 mirror(vec2 u) {
    if(u.x > 1.)
        u.x = 1. - fract(smin(-u.x, u.x, 0.5));
    if(u.x < 0.)
        u.x = fract(smin(-u.x, u.x, 0.5));
    if(u.y > 1.)
        u.x = 1. - fract(u.y);
    if(u.y < 0.)
        u.x = fract(u.y);
...
vec4 renderMainImage() {
    ...
    Q = (exp(-length(p - o) + 1.)) * (cos(-.1 * smoothTimeC * 0.2 + .1 * z + .45 * vec4(1, 2, 3, 4))) * .5 * (dot(reflect(n, d), q) - dot(n, d));
    Q *= Q;
    return Q;
}
