...
vec4 renderPassA() {
    ...
    float dangl = (pixel(ch1, sleft).x - pixel(ch1, sright).x)*(1.0-pow(syn_MidLevel*0.75, 2.0)/PI);
    U.z += dt*sst*tanh(3.*dangl);
   
    vec2 pvel = pspeed*vec2(cos(U.z), sin(U.z)) + 0.1*(hash22(U.xy+smoothTime*0.01)-0.5);;
...
vec4 renderMainImage() {
    ...
    vec4 pheromone = (2.65+pow(syn_BassLevel*0.5+syn_Intensity*0.5, 1.5)*(1.0+syn_Presence))*texel(ch1, pos);
