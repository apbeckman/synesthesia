...
vec2 path(in float x) {
    ...
    return vec2(_noise(x * 0.25) * .18 + _noise(x * 0.15) * .125, sin(x * 0.25) * 1.2 + sin(x * 0.15)); // Perturbing "X" and "Y."
...
float Menger(in vec3 q) {
    ...
    p = abs(fract(q / s) * s - s * .5);
    d = max(d, min(max(p.x, p.y), min(max(p.y, p.z), max(p.x, p.z))) - s / 4.); //- .015 
...
float map(in vec3 p) {
    ...
    vec3 pMenger = mix (p, vec3(0.0), justTubes);
    float walls = Menger(pMenger);
    ...
    p += vec3(sign(p.x) * (-.11 + (sin(p.z * PI * 2. + 1.57 / 1.)) * .05), 0., 0.); // Screen curve, and repositioning.
...
float dotPattern(vec2 p) {
    ...
    float c = dot(sin(p / 4. - cos(p.yx / .2 + smoothTimeC * 0.1)), vec2(.5));
...
vec4 renderMainImage() {
	...
	// u += _uvc*u*PI*PI*mirror.xy;
    // u = mix(u, abs(vec2(_noise(u/10. + TIME))), 0.25);
    u.xy = mix(u.xy, u.xy + _uvc * 0.125, fov);
	// Camera Setup.
    vec3 ro = camPath(bass_time * .5); // Camera position, doubling as the ray origin.
    ...
    float FOV = 1.25 * (1.0 + fov); // FOV - Field of view.
    vec3 fwd = normalize((lk) - ro);
    fwd.xy += mix(vec2(0), _uvc * fov *0.5* (1.0 - 0.3 * dot(_uvc, _uvc)), fov);
    ...
    rd.xz = _rotate(rd.xz, rotateXY.x * PI);
    rd.yz = _rotate(rd.yz, rotateXY.y * PI);

    float inv_screen = mix(1.0, -1.0, invert_screen);
// 
    float x_mirror = smin(rd.x * -1, rd.x, 0.6);

    float y_mirror = smin(rd.y * -1, rd.y, 0.6);

    vec2 mxy = vec2(x_mirror, y_mirror) * inv_screen;

    vec2 m = vec2(mirror_x, mirror_y);
    rd.xy = vec2(mix(rd.x, mxy.x, m.x), mix(rd.y, mxy.y, m.y));
    rd.xy = _rotate(rd.xy, rotate * PI);
    rd.xz = _rotate(rd.xz, rotategeo.x * PI);
    rd.yz = _rotate(rd.yz, rotategeo.y * PI);
    ...
    if(t < FAR) {
        ...
        if(saveObjID < 2.5)
            nor = texBump(sq, pOffs * ts, nor, 0.002 + step(saveObjID, 1.5) * 0.012);

        col = tex3D(sq, pOffs * ts, nor); // Texture pixel at the scene postion.
        col = smoothstep(-.3, .8, col) * vec3(1., .8, .7); // Process the color a little.
        // col = _grayscale(col, .750);
        col = _contrast(col, 1.1);
        ...
        float spot = max(2. - length(pOffs.xy - vec2(0, 1)), 0.) * (sin((pOffs.z) * PI + 1.57) * .5 + .5);
        ...
        if(saveObjID > 1.5) { 
            ...
            col = vec3(1) * dot(col * .9, vec3(.299, .587, .5114));
            // Add some fake reflection. Not reliable, but it's subtle.
            rCol = tex3D(sq, (pOffs + reflect(rd, nor)) * ts, nor);
            // rCol = tex3D(sq, p)
            col += rCol * .25 + spot * .125;
            
            spe2 = spe * spe * .125; // Ramp up the global specular a bit.
            if(saveObjID < 2.5)
                dif = pow(dif, 1.25) * 1.125;
        ...
        if(saveObjID > 2.5) {
            ...
            if(_isMediaActive() == true) {
                col = _textureMedia(pOffs.zy * 2. + 0.5).xyz;
            } else {
            ...
            // col += rCol*rCol*.25; // Screen reflection.            
        ...
        if (saveObjID < 2.5) {
            col = _hueRotate(col, 270);
            col = _grayscale(col, .75);
            }
