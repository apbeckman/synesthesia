...
vec3 path(float z) {
    z *= 0.14;
    return vec3(sin(z + cos(z) * 0.2), cos(z + sin(z * 0.3) * 0.25), 0.) * .9;
}
...
vec3 getRd(vec3 ro, vec3 lookAt, vec2 uv) {
    ...
    uv += _noise(_uv * PI + TIME * 0.2) * warp;
    float m_x = smin(uv.x*-1, uv.x, 0.5);
    float m_y = smin(uv.y*-1, uv.y, 0.5);
    vec2 rdm = vec2(m_x, m_y);
    uv.xy = mix(uv.xy, rdm, vec2(mirror_x, mirror_y));
...
vec3 getNormal(vec3 p) {
    vec2 t = vec2(0.001, 0.);
    return normalize(map(p).x - vec3(map(p - t.xyy).x, map(p - t.yxy).x, map(p - t.yyx).x));
}
...
vec4 renderPassA() {
    ...
    vec2 fragCoord = _xy;
    vec2 mirror_in = vec2(mirror_x, mirror_y);
    ...
    lookAt.xy = mix(lookAt.xy, lookAt.xy + _uvc * PI, Fov);
    float l_x = smin(lookAt.x * -1, lookAt.x, 0.5);
    float l_y = smin(lookAt.y * -1, lookAt.y, 0.5);
    vec2 lam = vec2(l_x, l_y);
    lookAt.xy = _rotate(lookAt.xy, Rotate * PI);
    lookAt.xy = mix(lookAt.xy, lam, mirror_in);
    lookAt += path(lookAt.z);


    lookAt.z += 2.;
    vec3 rd = getRd(ro, lookAt, _uvc * (1. + wave * 0.016));

    ro -= rd * texture(image30, (uv) * 256.).x * 0.2; // remove banding from glow

    //rd.xy *= rot(sin(script_time*0.12 + sin(script_time*0.14)*0.125 )*0.1);
    rd.xy = mix(rd.xy, rd.xy + _uvc, Fov);
    float m_x = smin(rd.x * -1, rd.x, 0.5);
    float m_y = smin(rd.y * -1, rd.y, 0.5);
    vec2 rdm = vec2(m_x, m_y);
    rdm = _rotate(rdm, Rotate*PI);
    rd.xy = _rotate(rd.xy, Rotate * PI);
    rd.xy = mix(rd.xy, rdm, mirror_in);
