...
float distFunc( vec3 point )
{
	...
	rotateAxis( point2.xy, (smoothTime+floor(point.z/repSpacing.z))/4.0 );
...
vec3 getShadedColor( vec3 hitPosition, vec3 normal, vec3 cameraPosition )
{
	...
	vec3 lightPosition = vec3(sin(smoothTime), 1.0, cos(smoothTime));
	...
	//	ambient component
    vec3 lightColor = vec3(abs(sin(smoothTimeB*0.84)), abs(cos(smoothTimeB)), abs(sin(smoothTimeB*1.337)))*0.5;
...
vec4 renderMainImage() {
	...
	camP += vec3( repSpacing.x/2.0+sin( TIME )*0.5, repSpacing.y/2.0+cos( smoothTime )*0.5, 6.0*smoothTime );
	vec3 camDir = calcCameraRayDir( 80.0, fragCoord.xy, RENDERSIZE.xy ); // Camera view direction
	...
	//	fog
	vec3 fogColor = vec3( 0.1, 0.1+0.4*(1.0-sin(TIME)), 0.1+0.5*(1.0-sin(TIME)) );
