...
// Make the random horizontal and vertical line direction more random, 
...
float hash31(vec3 p){
    p = mod(p, gSc);
    return fract(sin(dot(p, vec3(27.609, 137.583, 57.781)))*43758.5453); 
}
...
#define ZERO min(0.0, FRAMECOUNT)
...
//mat2 rot2(in float a){ float c = cos(a), s = sin(a); return mat2(c, -s, s, c); } * edited
...
vec4 renderMainImage() {
    ...
    vec2 ra = vec2(1, RENDERSIZE.x/RENDERSIZE.y);
    uv *= 1. + (dot(uv*ra, uv*ra)*.05 - .025);
	

    // Ray origin.
	vec3 ro = vec3((smoothTime*0.075), 0, -1); 
    // "Look At" position.
    vec3 lk = ro + vec3(.01*0., .01*0., .1);//vec3(0, -.25, TIME);  
    ...
    vec3 rgt = normalize(vec3(fwd.z, 2, -fwd.x )); 
        ...
        if(t<FAR){
           ...
           vec3 oCol;
            ...
            if(svObjID == 0.) {
                ...
                // Block coloring.
                vec3 tx = getTex(image3, uv + hash21(svGID.yz));
                ...
                rndF = smoothstep(.92, .97, sin(6.2831*rndF)*(.5 + sin(smoothTimeC*0.6)*10.) + .5);
                ...
                // Face pattern.
                float pat = fPat(uv, 1./.125, svGID.yz + faceID/(6.+pow(syn_HighHits*0.01, 2)) + svBPID/6.);
