...
float cellTile(in vec3 p){
    ...
    float cell_texture1 = min(v.z, v.w) - min(v.x, v.y); // First minus second order, for that beveled Voronoi look. Range [0, 1].
    float cell_texture2 = min(v.x, v.y); // Minimum, for the cellular look.
    d.x = mix(cell_texture1,cell_texture2,cell_mode);
    d.x = mix(d.x,0.0,Cartoonize);
    return d.x; // Normalize... roughly.

}

// The path is a 2D sinusoid that varies over time, depending upon the frequencies, and amplitudes.
// MS - Leaving this for now but this affects part of the view -- probably useful for camera
...
float smaxP(float a, float b, float s){
    ...
    float h = clamp( 0.5 + 0.5*(a-b)/s, 0., Walls);
...
// MS: Adds some lighting that looks cool with hits
float bumpSurf3D( in vec3 p){
    ...
    return bmp*syn_HighHits*Lighting_Gain;
...
float thickness( in vec3 p, in vec3 n, float maxDist, float falloff )
{
    ...
    return clamp( 1.-ao/nbIte + syn_OnBeat*Lighting_Gain, 0., 1.);
}
...
float calculateAO( in vec3 p, in vec3 n )
{
    ...
    return clamp(1.- ao/nbIte + Shadows, 0., 1.);
}
...
vec3 eMap(vec3 rd, vec3 sn){
    ...
    return smoothstep(0., 1., texCol+syn_MidLevel*Lighting_Gain*.15); // MS: This controls the lighting across the whole veins
...
vec4 renderMainImage() {
  ...
  // Original: vec3 lookAt = vec3(0, 1, TIME*2. + 0.1);
	...
	if(t<FAR){
	    ...
	    vec3 sp = t * rd+camPos - syn_BassLevel*Lighting_Gain*.1; // MS: Weird texture w/ swimming lines
    	...
    	// Ambient light.
	    float ambience = Ambience + syn_HighHits*5; // MS: Control ambient lighting w/ Ambience slider && high hits
        ...
        if(saveID>.5){ // Tunnel walls.
            ...
            texCol = vec3(.35, .25-syn_BassHits*Lighting_Gain*.2*Light_Hits, .2)*(1-syn_BassHits*.1*Light_Hits-smoothstep(-.1, .25, ct)) + vec3(Red, Green, Blue);
        }
