...
float terrainHigh( vec2 p )
{
    ...
    t +=   0.05*textureLod( image4, 0.001*q, 0.0 ).x;
    t +=   0.03*textureLod( image4, 0.005*q, 0.0 ).x;
    t += t*0.03*textureLod( image4, 0.020*q, 0.0 ).x;
...
float tubesH( vec3 pos, float time )
{
    ...
    t += 1.0*texture( image9, 0.01*pos.yz ).x;
    t += 2.0*texture( image4, 0.005*pos.xy ).x;
...
vec4 renderMainImage() {
    ...
    vec2 xy = -1.0 + 2.0*fragCoord.xy/RENDERSIZE.xy;
    vec2 sp = xy*vec2(RENDERSIZE.x/RENDERSIZE.y,1.0);

    //--------------------------

    // animate    
    float camid = floor((0.0+TIME-0.0)/9.0);
    float time = 16.5 + (0.0+TIME-0.0)*0.1 + 20.0*iMouse.x/RENDERSIZE.x + 72.1*camid + 19.0*max(0.0,camid-1.0);
    ...
    if( res.x>tmax )
    {
		...
		col = mix( col, 0.25*vec3(0.5,0.9,1.0), 0.4*smoothstep(0.0,1.0,texture(image4,0.000005*sc).x) );
	...
	else
	{
        ...
        col *= 0.2 + sqrt( texture( image4, 0.01*pos.xy*vec2(0.5,1.0) ).x *
                           texture( image4, 0.01*pos.zy*vec2(0.5,1.0) ).x );
        vec3 col2 = vec3(1.0,0.2,0.1)*0.01;
        col = mix( col, col2, 0.5*res.y );
        
        // grass
        float s = smoothstep(0.6,0.7,nor.y - 0.01*(pos.y-20.0));        
        s *= smoothstep( 0.15,0.2,0.01*nor.x+texture(image4, 0.001*pos.zx).x);
        vec3 gcol = 0.13*vec3(0.22,0.23,0.04);
        gcol *= 0.3+texture( image30, 0.03*pos.xz ).x*1.4;
        col = mix( col, gcol, s );
        //col *= texture( image4, 0.3*pos.xz ).x*3.2;
        nor = mix( nor, sor, 0.3*s );
        vec3 ptnor = nor;

        // trees
        s = smoothstep(0.9,0.95,nor.y - 0.01*(pos.y-20.0));        
        s *= smoothstep( 0.1,0.13,-0.17+texture(image4, 0.001*pos.zx).x);
        vec3 tor = -1.0 + 2.0*texture( image30, 0.015*pos.xz ).xyz;
        tor.y = 1.5;
        tor = normalize(tor);
		...
		// snow
        s = ptnor.y + 0.008*pos.y - 0.2 + 0.2*(texture(image30,0.00015*pos.xz+0.0*sor.y).x-0.5);
    ...
    // camera fade
    col *= smoothstep( 0.0, 0.1, 2.0*abs(fract(0.5+TIME/9.0)-0.5) );
